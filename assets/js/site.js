...
(function () {
  ...
  /* ------------------------------------------------------------------
     ...
     The theme itself is applied by an inline script in the head, before
     the page paints, so a reader who chose light does not get a black
     flash on every navigation. All this does is build the switch and
     write the choice down.

     Dark is the default and prefers-color-scheme is deliberately not
     consulted: somebody whose phone is in light mode has not asked for
     a different brand, they have asked for their phone to be brighter.
     ------------------------------------------------------------------ */
  var SUN =
    '<svg class="is-sun" viewBox="0 0 24 24" fill="none" stroke="currentColor"'
  ...
  var MOON =
    '<svg class="is-moon" viewBox="0 0 24 24" fill="currentColor" aria-hidden="true">'
  ...
  function applyTheme(name) {
    ...
    Array.prototype.forEach.call(
      ...
      function (b) {
        ...
        b.setAttribute('aria-label', name === 'light' ? 'Switch to dark' : 'Switch to light');
      }
    );
  }
  ...
  Array.prototype.forEach.call(
    document.querySelectorAll('[data-theme-toggle]'),
    function (btn) {
      if (!btn.innerHTML.trim()) btn.innerHTML = SUN + MOON;
  ...
  // sync the buttons with whatever the head script already applied
