 ...
 */
export function resolveArt(slot, { own = null, stock = {}, cut = {}, seed = 1 } = {}) {
  const role = slot.role ?? slot.id;
  // the slot's own id joins the seed, or three object slots on one sheet
  // all resolve to the same photograph
  const salt = [...String(slot.id ?? '')].reduce((h, c) => (h * 31 + c.charCodeAt(0)) | 0, 7);
  const at = (n, k = 0) => Math.abs((Math.floor(seed) + salt + k) % n);

  // an explicit map from the studio still wins outright
  if (own && own[role]) return { url: own[role], from: 'own', role };
  ...
  /* His own, before stock, and chosen by what the PHOTOGRAPH may do
     rather than by a list. Only some of the four can be cut and only
     some can be used whole, so asking a slot which it needs and
     filtering on that is the difference between his face on the sheet
     and his face wrecking it. `solid` says whether the ground behind is
     flat, which phone-chair requires: a clean key with no border
     disappears into anything busier. */
  const solid = slot.over !== 'art' && slot.treat !== 'over';
  const mine = ownFor(role, { wantsCut, solid });
  if (mine.length) {
    const p = mine[at(mine.length)];
    return { url: p.url, from: 'own', role, photo: p.name,
             cut: wantsCut ? p.cut : null, snap: p.snap, lean: p.lean,
             stands: !!p.stands, floats: !!p.floats };
  }
