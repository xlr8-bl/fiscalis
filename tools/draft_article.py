...
Drafts land in content/drafts/, NOT in content/articles/, so nothing reaches
the site until a person reads it and moves the file. That gate is deliberate.
...
If you want it to publish unattended anyway, set PUBLISH_WITHOUT_REVIEW=1 and it
will write straight to content/articles/. Read the docstring above twice first.
"""
...
def main(argv: list[str]) -> int:
    ...
    unattended = os.environ.get("PUBLISH_WITHOUT_REVIEW") == "1"
    if unattended and problems:
        print("Refusing to publish unattended: " + "; ".join(problems), file=sys.stderr)
        unattended = False

    dest_dir = ARTICLES if unattended else DRAFTS
    dest_dir.mkdir(parents=True, exist_ok=True)
    dest = dest_dir / f"{slug}.md"
    dest.write_text(text + "\n", encoding="utf-8")

    if topic_index is not None:
        mark_topic(topic_index, "[done]" if unattended else "[drafting]")
    ...
    if not unattended:
        print(
            "\nThis is a draft. Read it, edit it, then publish with:\n"
            f"  mv {rel} content/articles/\n"
            "  python3 tools/build_journal.py\n"
            "and mark the topic [done] in content/topics.md."
        )
    return 0
