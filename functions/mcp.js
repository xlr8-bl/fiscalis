...
/*
 * The two other ways to make a carousel, shut unless a person opens them.
 *
 * There were three roads and Spark took a different one each time:
 * plan_carousel filed slides only an image model could draw, and
 * design_carousel filed setup-and-payoff panels on device mockups, which
 * render perfectly and look like somebody else's account. Only
 * teach_carousel produces the templates.
 *
 * Out of the default tool scope was not enough on its own: a deployment
 * whose `agent.scope` is `everything` still lists them, and Spark called
 * them. So this is the refusal, and it names the one road rather than
 * only saying no.
 */
const otherEnginesShut = async (db, tool, why) => {
  if (String((await getSetting(db, 'agent.other_engines')) ?? '') === '1') return null;
  return toolFailed(
    `${tool} is switched off. ${why} Every carousel here is made with `
    + 'teach_carousel: you pick a template per slide and the site draws it on a '
    + 'canvas, with measured type on a ground whose contrast is known. Call '
    + 'next_carousel for the work order, then teach_carousel. (A person can open '
    + 'this path under Settings, What Spark can do.)'
  );
};
...
async function runTool(name, args, env) {
  ...
  switch (name) {
    ...
    case 'plan_carousel': {
      const shut = await otherEnginesShut(db, 'plan_carousel',
        'It files slides that are a generated picture, so they need an image '
        + 'model and cannot be drawn on the canvas at all.');
      if (shut) return shut;
    ...
    case 'design_carousel': {
      const shutD = await otherEnginesShut(db, 'design_carousel',
        'It draws setup-and-payoff panels on device mockups. They render fine '
        + 'and they are not the templates: a set made this way looks like a '
        + 'different account.');
      if (shutD) return shutD;
      const title = clean(args.title) || clean(args.topic) || 'Untitled carousel';
