...
/**
 * Which of the two drawing paths runs.
 *
 * 'workers' draws backgrounds on Cloudflare inside the free daily
 * allowance and the studio typesets the copy over them. 'gemini' calls
 * Google, which draws the copy too and costs money per slide.
 *
 * The default is the free one. A path that needs a card before it draws
 * anything is not a default, and the paid one is still one tap away.
 */
export const PROVIDERS = ['workers', 'gemini'];

export async function drawProvider(db, env, { getSetting } = {}) {
  if (db && getSetting) {
    const stored = String((await getSetting(db, 'draw.provider')) || '').trim();
    if (PROVIDERS.includes(stored)) return { provider: stored, source: 'studio' };
  }
  const fromEnv = String(env.DRAW_PROVIDER || '').trim();
  return PROVIDERS.includes(fromEnv)
    ? { provider: fromEnv, source: 'environment' }
    : { provider: 'workers', source: 'default' };
}
