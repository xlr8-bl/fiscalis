...
export function missingDatabase() {
  return htmlResponse(
    '<!DOCTYPE html><html lang="en"><head><meta charset="utf-8">' +
      ...
      '<p>Or sign in at <code>/studio.html</code> and press Set up.</p>' +
...
/**
 * Run a query, and treat "there are no tables yet" as unfinished setup
 * rather than as a failure. Anything else is a real error and rethrows.
 */
export async function orNotReady(fn) {
  try {
    return { ok: true, value: await fn() };
  } catch (e) {
    if (/no such table/i.test(String(e?.message ?? e))) return { ok: false };
    throw e;
  }
}
