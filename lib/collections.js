...
export const SETTINGS = [
  ...
  {
    group: 'What Spark can do',
    fields: [
      { name: 'agent.scope', type: 'select', label: 'What it can reach',
        options: [['carousel', 'Carousels only'], ['everything', 'Carousels and the journal']],
        help: 'Carousels only is the short list: making one, and nothing else. '
            + 'Nothing in it publishes, so Gemini never stops to ask permission. '
            + 'The journal tools are not deleted, only out of reach — switch to '
            + 'the second option to write articles again.' },
      { name: 'agent.picture_path', type: 'select', label: 'Picture slides',
        options: [['0', 'Off: the site draws every slide'],
                  ['1', 'On: it may also file generated pictures']],
        help: 'Off, every carousel is drawn here on a canvas: measured type on a '
            + 'flat ground whose contrast is known, free, no image model. On, '
            + 'Spark can also file slides that ARE a generated picture, and those '
            + 'cannot be drawn at all without Workers AI bound or a Gemini key. '
            + 'Off is why a carousel costs nothing to draw. It was reaching for '
            + 'the older path on its own and filing carousels nothing could draw, '
            + 'so this is a refusal rather than a preference.' },
    ],
  },
  {
    ...
    fields: [
      { name: 'post.route', type: 'select', label: 'Which road they take',
        ...
        help: 'Straight means my own apps talk to Instagram and TikTok, '
            + 'which needs each one to have approved the app. '
