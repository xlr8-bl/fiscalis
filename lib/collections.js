  ...
  },
  {
    ...
    fields: [
      { name: 'post.route', type: 'select', label: 'Which road they take',
        options: [['direct', 'Straight to the platforms'], ['ayrshare', 'Through Ayrshare']],
        help: 'Straight means my own apps talk to Instagram, TikTok and '
            + 'Facebook, which needs each one to have approved the app. '
            + 'Ayrshare posts to all three in one call using their apps '
            + 'instead, so nothing waits on a review. It needs '
            + 'AYRSHARE_API_KEY set as a Cloudflare secret, it has no free '
            + 'plan, and the trial runs 28 days. Switch back the day TikTok '
            + 'approves: nothing else changes, and no redeploy.' },
    ],
  },
  {
