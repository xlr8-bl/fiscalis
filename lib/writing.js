...
export async function writingBrief(db) {
  ...
  return {
    ...
    subjects_left: left.map((x) => ({
      id: x.id, pillar: x.pillar, angle: x.angle,
      research: x.look_for, picture: x.picture,
    })),
    ...
    pillars: {
      ...
      rule: 'Never take two subjects from the same pillar in one run, and do not '
          + 'take the pillar the last run ended on. writing_run already picks this '
          + 'way; if you are choosing by hand, choose the same way.',
    },

    how_to_pick:
      'Work down subjects_left rather than browsing it, but never two from one '
      + 'pillar in a sitting. If the list is empty, go back through '
      + 'already_written and find one where something has actually changed since '
      + 'it was written, and say what changed.',
...
export async function writingRun(db, { force = false } = {}) {
  ...
  /* One per pillar, round robin, starting after the pillar the last run
     ended on. Taking the first N off the list gives five `craft` articles
     in a morning, and five articles about the same corner of the subject
     is how a journal turns into a note. */
  const { getSetting, putSetting } = await import('./tokens.js');
  const lastPillar = (await getSetting(db, 'writing.pillar')) || '';
  const order = PILLARS.slice(PILLARS.indexOf(lastPillar) + 1)
    .concat(PILLARS.slice(0, PILLARS.indexOf(lastPillar) + 1));
  const byPillar = new Map(order.map((k) => [k, brief.subjects_left.filter((x) => x.pillar === k)]));
  const take = [];
  for (let round = 0; take.length < plan.count; round++) {
    let tookAny = false;
    for (const k of order) {
      const pool = byPillar.get(k);
      if (!pool || pool.length <= round) continue;
      take.push(pool[round]);
      tookAny = true;
      if (take.length >= plan.count) break;
    }
    if (!tookAny) break;
  }
  ...
  /* An empty bank stops the run rather than filling it.
     At five a day an eighteen-subject bank is gone in four days, and the
  ...
  if (!take.length) {
    return {
      ...
      why: 'Every subject in the bank has been written about. Nothing is being '
        + 'written today rather than something being repeated. Add subjects, or '
        + 'go through already_written and find one where something has actually '
