...
/* Both late columns go through the same fallback. A scheduled article
   has to show as scheduled in the studio, which means the list query
   carries the time. */
const WITH_COVER = `${COLUMNS}, cover, publish_at`;
...
async function selecting(db, build, bind, { one = false } = {}) {
  ...
  try {
    ...
    if (!got) return got;
    const bare = { cover: '', publish_at: '' };
    return one ? { ...got, ...bare } : got.map((r) => ({ ...r, ...bare }));
  }
}
