...
/**
 * Is the picture actually being served, and as what?
 *
 * HEAD first: the answer is in the headers and a 4K master is megabytes.
 *
 * But a HEAD that fails is not proof the picture is missing, and this is
 * not hypothetical — /media only exported onRequestGet, so every HEAD on
 * the site 404'd while every GET returned the file. That route answers
 * HEAD now, and this still does not trust a bad one: anything other than
 * a clean answer is retried as a ranged GET for the first byte, which is
 * exactly what the platforms will do and costs one byte to find out.
 */
async function reachable(url, fetcher) {
  const read = (res) => ({
    ok: true,
  ...
  } catch { /* fall through to the byte */ }

  try {
    const res = await fetcher(url, { method: 'GET', redirect: 'follow', headers: { range: 'bytes=0-0' } });
    if (!res.ok && res.status !== 206) return { ok: false, status: res.status };
    // content-length on a 206 is the range, not the file, so it is dropped
    return { ...read(res), bytes: res.status === 206 ? null : read(res).bytes };
  } catch (e) {
 ...
 */
async function checkMedia(site, slides, targets, fetcher) {
  const out = [];
  if (!site) {
    return [stop('the picture URLs', 'SITE is not set, so there is no absolute URL to give the platforms.')];
  }

  const urls = slides.map((s) => `${site}/media/${s.media_key}`);
  const checks = await Promise.all(urls.map((u) => reachable(u, fetcher)));
  ...
  out.push(missing.length
    ? stop('the pictures are reachable',
      `${missing.join(', ')}. Meta and TikTok fetch by URL, so a slide they cannot `
      + 'get is a post that does not happen.')
    : ok('the pictures are reachable', `all ${checks.length} answered.`));
...
export async function preflight(env, row, { fetcher = fetch } = {}) {
  ...
  if (slides.length < 2) {
    ...
    checks.push(...await checkMedia(site, slides, targets, fetcher));
  }
  ...
  if (route === 'buffer') {
    checks.push(ok('the road', 'Buffer, one call per channel.'));
