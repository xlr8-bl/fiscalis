 ...
 *
 * WHY THE CUT-OUT ROLES HAVE NO STOCK, WHICH WAS MEASURED AND NOT ASSUMED
 *
 * Half the corpus wants cut-outs: a figure standing in front of type, a
 * row of objects on bare paper. The keyless library is Wikimedia
 * Commons, which is an archive of photographs of the world rather than a
 * stock library, and photographs of the world have backgrounds.
 *
 * Fifteen candidates were fetched and run through tools/extract_objects.py.
 * Median keyability 0.41; none reached the 0.7 the keyer calls usable.
 * Adding "white background" to the query does not help, because an
 * archive matches the words loosely and returns a phone box on a street.
 *
 * So cut-out roles do not get stock. They get one of two things:
 *
 *   drawn      the interface and hardware objects are DRAWN, in code. A
 *              drawn phone is a real cut-out with a clean edge, it costs
 *              nothing, and unlike a photograph of a 2019 handset it does
 *              not date. Several references are illustrations anyway.
 *   the box    the figure role has no drawn substitute, because it is
 *              Ashley. It draws its box at the size he will fill, with the
 *              brief in it, until he shoots. PHOTOS.md has the cut-out
 *              rules, and they are about the shoot rather than the software:
 *              a plain even wall is the difference between 0.4 and 0.95.
 *
 * Do not re-attempt stock cut-outs against a keyless archive. It has been
 * tried and measured. If it is ever worth solving, the answer is an
 * UNSPLASH_ACCESS_KEY, which reaches a real stock library.
 */
...
export const ROLES = {
  ...
  figure: {
    ...
    reject: /\b(dog|hound|horse|statue|monument|cattle|sheep)\b/i,
    shape: 'tall',      // a whole person standing is taller than they are wide
    fill: 'box',        // no stock: it is him
  ...
  scene: {
    ...
    search: ['empty restaurant interior', 'coffee shop counter morning',
             'shop front street', 'desk laptop window light'],
  ...
  objects: {
    ...
    reject: /\b(wheat|field|crop|harvest|fire|landscape|sky|mountain|museum)\b/i,
    shape: 'any',       // an object on a plain ground is whatever shape it is
    fill: 'drawn',      // a row of drawn objects keys perfectly and never dates
    draws: ['phone', 'crt', 'handset'],
  ...
  object: {
    ...
    reject: /\b(wall of|many|crowd)\b/i,
    shape: 'any',
    fill: 'drawn',
    draws: ['crt'],
 ...
 */
export function resolveArt(slot, { own = {}, stock = {}, seed = 1 } = {}) {
  const role = slot.role ?? slot.id;

  // 1. his own, always, the moment it exists
  if (own[role]) return { url: own[role], from: 'own', role };

  /*
   * 2. cached stock — but only where a rectangle is the right answer.
   *
   * `fill: 'box'` means the slot is a CUT-OUT: a figure standing on the
   * paper, overlapping the type. A rectangular photograph dropped in there
   * does not read as a stand-in for a cut-out, it reads as a photograph
   * pasted onto the sheet, which is worse than an empty box because it
   * looks finished and wrong rather than unfinished and honest.
   *
   * This was ignored at first, and h051 rendered a landscape scene where
   * the reference has a man walking through the word.
   */
  const n = ROLES[role]?.fill === 'box' ? 0 : stock[role] ?? 0;
  if (n > 0) {
    // chosen by the seed so it is stable across renders, but two specs
    // filed on the same day are not twins
    const pick = Math.abs(Math.floor(seed)) % n;
    return { url: stockPath(role, pick), from: 'stock', role };
  }
