...
export const LAYOUTS = {
  ...
  h051: {
    ...
    slots: [
      ...
      /* Re-measured off the reference: the walking man is x .343-.600 and
         stands on the rule at .80, which is a 0.6 aspect. The batched box
         was half as wide again, so a wide cut-out filled it and ran across
         the body column. `shapeTol` now refuses anything that shape. */
      /* Mirrored, stood on the floor rule at 0.806, and filling the
         left of the sheet up to the type column at 0.575. */
      { id: 'figure', t: 'art', own: 'sky-arms', pin: false,
        box: [0.055, 0.350, 0.420, 0.456],
        treat: 'contain', anchor: 'bottom', shapeTol: 0.30,
  ...
  /* ------------------------------------------------------------- h013
   * Measured off .refs/hooks/h013.jpg (736x920, 0.80), tighten.py where the
   * concrete ground let it converge and the printed grid where it did not:
   *   scene  full bleed
   *   date   .875/.058 .082x.027 (2 lines, right to .957)
   *   rail   .942/.158 .015x.178, turned 90
   *   kicker .102/.079 .075x.030      head  .111/.121 .418x.122 (cap .094)
   *   star   .067/.224 .092x.076      turn  .173/.234 .363x.066 (cap .066)
   *   strap  .109/.326 .371x.012
   *   reqHead .554/.354 .130x.011     reqList .554/.367 .351x.035 (pitch .0127)
   *   deadHead .109/.540 .081x.011    deadList .073/.554 .117x.035, right
   *   foot   .344/.959 .315x.013      mark .048/.918 .077x.052
   * Everything is set in the obliqued geometric; there is no upright type on
   * the sheet above the small print.
   */
  h013: {
    id: 'h013', family: 'figure-cut', ground: 'paper', ref: 'h013', src: 0.80,

    does: 'A job advert for something that is not a job. A photograph fills the '
        + 'whole sheet — someone lying on the floor of it, doing the thing being '
        + 'advertised — and the notice is set straight over the top in an obliqued '
        + 'geometric: a small opening word, one enormous verb, then the object of '
        + 'the verb again in the accent colour behind a display asterisk. Around '
        + 'it, in small print, the apparatus of a real posting: a date, a rail '
        + 'reading APPLY NOW, requirements, a deadline, and a line at the foot '
        + 'saying how to reply. The joke is entirely in the form. Works for '
        + 'anything you can frame as a vacancy, a call, or a set of conditions.',
    avoid: 'The small print is where the humour lives and the headline only sets '
         + 'it up, so a sheet with a funny headline and vague requirements has it '
         + 'backwards. Nothing urgent or serious: the form is playful and will '
         + 'undercut you. And the photograph has to be light and flat under the '
         + 'type — this is a dark notice printed on pale concrete, and a busy or '
         + 'dark picture buries every one of these blocks.',

    slots: [
      { id: 'scene', t: 'art', role: 'scene', box: [0.000, 0.000, 1.000, 1.000],
        tint: 'ground', key: 0.52, contrast: 0.86, polarity: 'keep',
        wants: 'One photograph filling the sheet, shot from above: a pale flat '
             + 'floor with a person lying on it doing the thing being advertised. '
             + 'It has to be light enough to take dark type across the top half '
             + 'and quiet enough to take small print in three corners.' },

      { id: 'date', t: 'type', box: [0.875, 0.052, 0.082, 0.034],
        role: 'grotesque', weight: '400', fill: 'mark', align: 'right',
        fit: 'shrink', size: 0.015, min: 0.010, leading: 1.20,
        wants: 'Two short lines of caps, top right, right-aligned. A date — the '
             + 'day and then the weekday, as a real notice would file it.' },
      { id: 'rail', t: 'type', box: [0.938, 0.158, 0.022, 0.178],
        role: 'grotesque', weight: '400', fill: 'mark', rotate: 90, track: 0.28,
        fit: 'shrink', size: 0.014, min: 0.010, align: 'center',
        wants: 'Two words in caps running down the right edge, widely letterspaced. '
             + 'The instruction: what the reader is meant to do about all this.' },

      { id: 'kicker', t: 'type', box: [0.102, 0.076, 0.120, 0.034],
        role: 'geometric', style: 'italic', weight: '700', fill: 'mark',
        fit: 'shrink', size: 0.042, min: 0.024,
        wants: 'One or two words that open the sentence the headline finishes. '
             + '"I am", "We are", "Still". Lower case, small, leaning.' },
      { id: 'head', t: 'type', box: [0.111, 0.121, 0.430, 0.122],
        role: 'geometric', style: 'italic', weight: '700', fill: 'mark',
        fit: 'shrink', size: 0.131, min: 0.070, track: -0.01, vAlign: 'top',
        wants: 'ONE word, the verb of the notice, set at an eighth of the sheet. '
             + 'Under eight characters or it will not hold the size.' },
      { id: 'star', t: 'star', box: [0.067, 0.224, 0.092, 0.076],
        fill: 'accent', count: 6, weight: 0.40, turn: 15 },
      { id: 'turn', t: 'type', box: [0.173, 0.230, 0.380, 0.074],
        role: 'geometric', style: 'italic', weight: '700', fill: 'accent',
        fit: 'shrink', size: 0.092, min: 0.050, track: -0.01, vAlign: 'top',
        wants: 'ONE word in the accent colour: the object of the verb, and the '
             + 'twist. The pairing is the whole hook — the first word is what '
             + 'anybody would say and this one is not.' },
      { id: 'strap', t: 'type', box: [0.109, 0.324, 0.400, 0.016],
        role: 'grotesque', weight: '700', fill: 'mark',
        fit: 'shrink', size: 0.014, min: 0.010,
        wants: 'One line of small bold under the headline. The condition of '
             + 'entry, said drily and without a wink.' },

      { id: 'reqHead', t: 'type', box: [0.554, 0.352, 0.180, 0.014],
        role: 'grotesque', weight: '700', fill: 'mark',
        fit: 'shrink', size: 0.012, min: 0.009,
        wants: 'One word and a colon. What the list under it is a list of.' },
      { id: 'reqList', t: 'type', box: [0.554, 0.366, 0.351, 0.038],
        role: 'grotesque', weight: '400', fill: 'mark',
        fit: 'shrink', size: 0.012, min: 0.009, leading: 1.12,
        wants: 'Three lines, each opening with a dash. Every one a specific '
             + 'complaint stated as though it were a reasonable requirement. This '
             + 'block carries the humour.' },

      { id: 'deadHead', t: 'type', box: [0.073, 0.538, 0.117, 0.014],
        role: 'grotesque', weight: '700', fill: 'mark', align: 'right',
        fit: 'shrink', size: 0.012, min: 0.009,
        wants: 'One word and a colon, right-aligned against the left margin.' },
      { id: 'deadList', t: 'type', box: [0.073, 0.552, 0.117, 0.038],
        role: 'grotesque', weight: '400', fill: 'mark', align: 'right',
        fit: 'shrink', size: 0.012, min: 0.009, leading: 1.12,
        wants: 'Three short lines, right-aligned. A limit expressed as a feeling '
             + 'rather than a date.' },

      { id: 'foot', t: 'type', box: [0.320, 0.956, 0.360, 0.016],
        role: 'grotesque', weight: '400', fill: 'mark', align: 'center',
        fit: 'shrink', size: 0.013, min: 0.010,
        wants: 'One line across the bottom. How to reply, in the same deadpan.' },
      { id: 'mark', t: 'type', box: [0.048, 0.948, 0.130, 0.024],
        role: 'pixel', weight: '400', fill: 'mark',
        fit: 'shrink', size: 0.016, min: 0.011,
        wants: 'The wordmark, bottom left, where the posting would be signed.' },
    ],
  },

  /* ------------------------------------------------------------- h014
   * Measured off .refs/hooks/h014.jpg (736x920, 0.80). Line boxes are set
   * from the BASELINE back (y = baseline - 0.78 x size), because the three
   * lines are stepped and it is the baselines that are even, not the tops:
   *   baselines .314 / .361 / .409, pitch .047
   *   lineA "different" ink .260/.283 .259x.032, size .043
   *   lineB "gets"      ink .253/.319 .171x.058, size .072
   *   lineC "noticed."  ink .312/.363 .394x.046, size .076
   *   mark   .774/.060 .152x.070
   *   badgeL .073/.926 .020x.013   footL .101/.926 .130x.011
   *   badgeR .716/.926 .011x.013   footR .730/.926 .197x.011
   */
  h014: {
    id: 'h014', family: 'photo-field', ground: 'navy', ref: 'h014', src: 0.80,

    does: 'A dark photograph seen from above, with one lit doorway in it and one '
        + 'small figure standing in the light. Over it, three lines that step to '
        + 'the right and grow as they go: the setup in white, then two lines in '
        + 'the accent colour, the last of them ending in a full stop. The third '
        + 'line lands on the doorway, so the sentence and the light arrive at the '
        + 'same place. Two contact lines sit along the foot. Use it for anything '
        + 'about being seen, leaving, starting, or being the only one doing a '
        + 'thing.',
    avoid: 'The picture is the argument, so a stock crowd shot with nothing '
         + 'singular in it leaves three lines of type floating on a blue '
         + 'rectangle. It needs one thing separated from everything else — one '
         + 'lit door, one figure, one gap. And the sentence has to break so the '
         + 'last line is the payoff; broken anywhere else the stair is decoration.',

    slots: [
      { id: 'scene', t: 'art', role: 'scene', box: [0.000, 0.000, 1.000, 1.000],
        tint: 'field', contrast: 1.12, darken: 0.28, polarity: 'keep',
        wants: 'A dark, wide, high-angle photograph with a single source of light '
             + 'in it and one figure at that light. Everything else in shadow. '
             + 'Shot from above rather than at eye level — the reader is looking '
             + 'down on the situation, which is what makes one person in it read '
             + 'as separate rather than as a subject.' },

      { id: 'mark', t: 'type', over: 'art', polarity: 'keep', box: [0.660, 0.058, 0.274, 0.040],
        role: 'pixel', weight: '400', fill: 'mark', align: 'right',
        fit: 'shrink', size: 0.026, min: 0.014,
        wants: 'The wordmark, top right, in the light colour.' },

      { id: 'lineA', t: 'type', over: 'art', polarity: 'keep', box: [0.260, 0.281, 0.262, 0.036],
        role: 'geometric', weight: '700', fill: 'mark', track: -0.01,
        fit: 'shrink', size: 0.043, min: 0.024,
        wants: 'The first word or two, small and white. The condition: what has '
             + 'to be true. One word is best.' },
      { id: 'lineB', t: 'type', over: 'art', polarity: 'keep', box: [0.253, 0.305, 0.175, 0.060],
        role: 'geometric', weight: '700', fill: 'accent', track: -0.015,
        fit: 'shrink', size: 0.072, min: 0.040,
        wants: 'The verb, half again as big, in the accent colour. One short '
             + 'word — it is set at a fourteenth of the sheet and there is no '
             + 'room for two.' },
      { id: 'lineC', t: 'type', over: 'art', polarity: 'keep', box: [0.312, 0.350, 0.396, 0.064],
        role: 'geometric', weight: '700', fill: 'accent', track: -0.015,
        fit: 'shrink', size: 0.076, min: 0.042,
        wants: 'The payoff, biggest, stepped furthest right, ending in a full '
             + 'stop. It has to be the word the reader came for — this line sits '
             + 'on the light in the picture and everything else is setup.' },

      { id: 'badgeL', t: 'badge', box: [0.073, 0.923, 0.019, 0.015],
        icon: 'phone', fill: 'mark', on: 'ground' },
      { id: 'footL', t: 'type', over: 'art', polarity: 'keep', box: [0.101, 0.924, 0.150, 0.014],
        role: 'grotesque', weight: '700', fill: 'mark',
        fit: 'shrink', size: 0.015, min: 0.010,
        wants: 'A phone number, bottom left, beside the disc.' },
      { id: 'badgeR', t: 'badge', box: [0.714, 0.923, 0.015, 0.012],
        icon: 'globe', fill: 'mark', solid: false },
      { id: 'footR', t: 'type', over: 'art', polarity: 'keep', box: [0.730, 0.924, 0.210, 0.014],
        role: 'grotesque', weight: '500', fill: 'mark',
        fit: 'shrink', size: 0.015, min: 0.010,
        wants: 'A web address, bottom right, beside the ringed globe.' },
    ],
  },
  ...
  h020: {
    ...
    slots: [
      ...
      /* One icon, in a gap the layout actually has. Chosen for what the
         sheet SAYS, and the copy may name a different one: the position
         is the design and the object is the argument. */
      { id: 'icon', t: 'art', icon: 'start', box: [0.062, 0.060, 0.115, 0.078],
        treat: 'contain',
        wants: 'One object from the pack, for what it means here. Change the '
             + 'name if a different one fits the words better.' },
    ],
  },
  ...
  h024: {
    ...
    slots: [
      ...
      /* `pin: false` because this sheet has decided where he goes. Without
         it the automatic bottom-right pinning for blue-flat took him to
         the corner and he ran straight past the floor rule. The box's
         foot IS the rule, at 0.826, so he stands on it, and its right
         edge IS the rule's right end at 0.912, so his cut side lands on
         a line that is actually drawn rather than stopping mid-sheet.
         Top dropped to 0.272 to clear the mark, which sits at 0.268. */
      { id: 'figure', t: 'art', role: 'figure', pin: false,
        box: [0.442, 0.272, 0.470, 0.554],
        treat: 'contain', anchor: 'bottom right',
      ...
      /* The sheet was a question, a figure and nothing else, which is
         restraint at reference size and an empty poster at posting size.
         What follows is what the question was ABOUT: the sides answer
         it, the rule under them closes the sheet, and neither crowds the
         figure because both sit outside its box. */
      /* One line across the sheet, not two either side of the figure.
         Two boxes of 0.20 could not hold a sentence and the right one
         clipped. Full width crossed the figure instead, because the
         figure runs from 0.29 to 0.76 and through the whole middle of
         the sheet: the only band free of it is the column beside it. So
         the answer is a caption in that column, which is a device this
         layout wanted anyway. */
      /* `wrap`, not `shrink`. shrink fits ONE line by making it smaller,
         which is why this had to be handed a list of lines and then read
         as four statements instead of one sentence. It is a sentence.
         The renderer breaks it; the writer does not. */
      { id: 'answer', t: 'type', box: [0.088, 0.330, 0.185, 0.260],
        role: 'body', weight: '500', fill: 'mark',
        fit: 'wrap', size: 0.019, min: 0.014, leading: 1.35,
        wants: 'ONE SENTENCE, written as one string. It wraps itself in the '
             + 'column beside the figure, so do not break it into lines and do '
             + 'not write four short statements: it is read as a sentence and '
             + 'a list here reads as four unrelated claims. The reason nobody '
             + 'else is going to do it. Plain, not clever.' },
      { id: 'floor', t: 'rect', box: [0.088, 0.826, 0.824, 0.0016], fill: 'mark' },
      { id: 'tag', t: 'type', box: [0.088, 0.842, 0.500, 0.030],
        role: 'grotesque', weight: '700', fill: 'mark',
        fit: 'shrink', size: 0.018, min: 0.014, track: 0.06,
        wants: 'The series, in capitals. Furniture, not a sentence.' },
      /* One icon, in a gap the layout actually has. Chosen for what the
         sheet SAYS, and the copy may name a different one: the position
         is the design and the object is the argument. */
      { id: 'icon', t: 'art', icon: 'cursor', box: [0.830, 0.300, 0.110, 0.078],
        treat: 'contain',
        wants: 'One object from the pack, for what it means here. Change the '
             + 'name if a different one fits the words better.' },
    ],
  ...
  /* ------------------------------------------------------------- h031
   * Measured off .refs/hooks/h031.jpg (736x920, 0.80) with tighten.py:
   *   headA .438/.321 .122x.034     headB .340/.360 .319x.070
   *   listA .310/.498 .380x.023     listB .292/.524 .401x.024
   *   listC .296/.554 .409x.022
   *   the five pills, which is the whole annotation device:
   *     .147/.351 .137x.042   .834/.356 .140x.040   .590/.422 .144x.041
   *     .114/.548 .145x.041   .717/.710 .141x.041
   * They are scattered rather than ranged, and three of the five overlap
   * the object in the middle. That overlap is what makes them read as
   * stuck on afterwards rather than as a legend.
   */
  h031: {
    id: 'h031', family: 'annotated-photo', ground: 'red', ref: 'h031', src: 0.80,

    does: 'One object photographed dead centre on a flood of colour, a question '
        + 'over it, the answer under it in three short lines, and numbers stuck '
        + 'around the whole thing on little dark pills. The metaphor and the '
        + 'proof arrive in the same image: the object says what the thing is '
        + 'LIKE, and the pills say it worked. For a method you can name in three '
        + 'parts and evidence in figures.',
    avoid: 'The three lines have to be genuinely parallel — same shape, same '
         + 'length, a quantity in each — or the middle of the sheet reads as a '
         + 'list somebody padded. And the numbers have to be real. This layout '
         + 'points at five of them at once, which is exactly the arrangement '
         + 'that makes an invented one obvious: nobody has five round numbers.',

    slots: [
      { id: 'scene', t: 'art', role: 'scene', box: [0.000, 0.000, 1.000, 1.000],
        polarity: 'keep',
        wants: 'One object, centred, filling the middle half of the frame, on a '
             + 'plain flooded ground with nothing else in it. It should be the '
             + 'metaphor rather than the subject: the thing the work is LIKE. '
             + 'Hands entering from the bottom edge are what stop it reading as '
             + 'a product shot, so a pair of them holding something is worth '
             + 'more than a cleaner picture without.' },

      { id: 'headA', t: 'type', over: 'art', polarity: 'keep',
        box: [0.400, 0.312, 0.200, 0.045],
        role: 'display', weight: '800', fill: 'photo', align: 'center',
        fit: 'shrink', size: 0.047, min: 0.030,
        wants: 'The first half of the question, caps. One or two words, and the '
             + 'shorter of the two lines.' },

      { id: 'headB', t: 'type', over: 'art', polarity: 'keep',
        box: [0.330, 0.355, 0.340, 0.080],
        role: 'display', weight: '800', fill: 'photo', align: 'center',
        fit: 'shrink', size: 0.078, min: 0.045,
        wants: 'The rest of the question, caps, ending in a question mark. It is '
             + 'the widest thing on the sheet and it sits on the object, so it '
             + 'has to be one word or two short ones.' },

      { id: 'list', t: 'type', over: 'art', polarity: 'keep',
        box: [0.285, 0.494, 0.430, 0.088],
        role: 'grotesque', weight: '700', fill: 'accent', align: 'center',
        fit: 'shrink', size: 0.030, min: 0.019, leading: 1.28,
        wants: 'THREE lines, caps, each one a quantity and a thing: a lot of X, a '
             + 'little Y, none of Z. They have to be the same grammatical shape '
             + 'and close to the same length, because they are read as one '
             + 'gesture rather than three points. The last one is the joke, so '
             + 'it goes last.' },

      { id: 'pillA', t: 'pill', solid: true, box: [0.147, 0.351, 0.137, 0.042],
        fill: 'mark', r: 'pill' },
      { id: 'eyeA', t: 'badge', icon: 'eye', ring: false,
        box: [0.161, 0.360, 0.024, 0.024],
        fill: 'accent' },
      { id: 'countA', t: 'type',
        box: [0.194, 0.361, 0.079, 0.022],
        role: 'grotesque', weight: '700', fill: 'accent', align: 'center',
        fit: 'shrink', size: 0.022, min: 0.013,
        wants: 'A number with a K or an M on it. It has to be a figure you can '
             + 'point at, never a rounded-off guess.' },

      { id: 'pillB', t: 'pill', solid: true, box: [0.834, 0.356, 0.140, 0.040],
        fill: 'mark', r: 'pill' },
      { id: 'eyeB', t: 'badge', icon: 'eye', ring: false,
        box: [0.848, 0.365, 0.022, 0.022],
        fill: 'accent' },
      { id: 'countB', t: 'type',
        box: [0.882, 0.366, 0.081, 0.021],
        role: 'grotesque', weight: '700', fill: 'accent', align: 'center',
        fit: 'shrink', size: 0.021, min: 0.012,
        wants: 'A number with a K or an M on it. It has to be a figure you can '
             + 'point at, never a rounded-off guess.' },

      { id: 'pillC', t: 'pill', solid: true, box: [0.590, 0.422, 0.144, 0.041],
        fill: 'mark', r: 'pill' },
      { id: 'eyeC', t: 'badge', icon: 'eye', ring: false,
        box: [0.604, 0.431, 0.023, 0.023],
        fill: 'accent' },
      { id: 'countC', t: 'type',
        box: [0.639, 0.432, 0.084, 0.021],
        role: 'grotesque', weight: '700', fill: 'accent', align: 'center',
        fit: 'shrink', size: 0.021, min: 0.012,
        wants: 'A number with a K or an M on it. It has to be a figure you can '
             + 'point at, never a rounded-off guess.' },

      { id: 'pillD', t: 'pill', solid: true, box: [0.114, 0.548, 0.145, 0.041],
        fill: 'mark', r: 'pill' },
      { id: 'eyeD', t: 'badge', icon: 'eye', ring: false,
        box: [0.129, 0.557, 0.023, 0.023],
        fill: 'accent' },
      { id: 'countD', t: 'type',
        box: [0.163, 0.558, 0.084, 0.021],
        role: 'grotesque', weight: '700', fill: 'accent', align: 'center',
        fit: 'shrink', size: 0.021, min: 0.012,
        wants: 'A number with a K or an M on it. It has to be a figure you can '
             + 'point at, never a rounded-off guess.' },

      { id: 'pillE', t: 'pill', solid: true, box: [0.717, 0.710, 0.141, 0.041],
        fill: 'mark', r: 'pill' },
      { id: 'eyeE', t: 'badge', icon: 'eye', ring: false,
        box: [0.731, 0.719, 0.023, 0.023],
        fill: 'accent' },
      { id: 'countE', t: 'type',
        box: [0.765, 0.720, 0.082, 0.021],
        role: 'grotesque', weight: '700', fill: 'accent', align: 'center',
        fit: 'shrink', size: 0.021, min: 0.012,
        wants: 'A number with a K or an M on it. It has to be a figure you can '
             + 'point at, never a rounded-off guess.' },
  ...
  h077: {
    ...
    slots: [
      ...
      { id: 'arrow', t: 'rect', box: [0.072, 0.450, 0.052, 0.042],
        r: 'pill', fill: 'accent' },
      { id: 'body', t: 'type', box: [0.072, 0.500, 0.330, 0.200],
        role: 'body', weight: '500', fill: 'mark',
      ...
      /* The crossed-legs one, pinned. It runs off no edge of its own
         frame so it floats, and a clean key with no border needs the
         solid ground this sheet has. Given the right half and the foot,
         because a seated figure squeezed into a tall slot reads as a
         thumbnail of itself. */
      { id: 'figure', t: 'art', own: 'phone-chair', box: [0.425, 0.290, 0.520, 0.615],
        treat: 'contain', anchor: 'bottom',
        wants: 'The seated figure, whole, keyed and floating. It fills the '
             + 'right of the sheet and sits on the foot.' },
      /* One icon, in a gap the layout actually has. Chosen for what the
         sheet SAYS, and the copy may name a different one: the position
         is the design and the object is the argument. */
      { id: 'icon', t: 'art', icon: 'message', box: [0.070, 0.300, 0.110, 0.078],
        treat: 'contain',
        wants: 'One object from the pack, for what it means here. Change the '
             + 'name if a different one fits the words better.' },
    ],
  ...
  /* ------------------------------------------------------------- h017
   * Measured off .refs/hooks/h017.jpg (736x981, 0.75):
   *   frame  .129/.038 .734x.132   stamp .145/.054 .693x.089 (cap height)
   *   burst  .118/.197 .810x.608   object .130/.245 .770x.530
   *   callout .615/.285 .200x.115  cry  .625/.320 .170x.055
   *   lineA  .042/.826 .512x.065 (baseline .887, size .077)
   *   lineB  .348/.904 .569x.055 (baseline .954, size .063)
   * One ink on one ground: everything is the ACCENT on the field, with no
   * black anywhere, which is what a risograph actually is. The object is
   * printed in the same ink (tint 'print'), not screened in black.
   */
  h017: {
    id: 'h017', family: 'risograph', ground: 'amber', ref: 'h017', src: 0.75,

    does: 'A one-colour zine page. A rubber stamp across the top shouting an '
        + 'instruction, a ragged starburst filling the middle with one obsolete '
        + 'object printed on it, a small hand-drawn callout with a single word in '
        + 'it, and then the turn written underneath in felt-tip across two lines '
        + 'that step to the right. The stamp gives the order and the handwriting '
        + 'takes it back — that gap is the joke, and it only works if the two '
        + 'are in different voices.',
    avoid: 'The object has to be dated and specific: a brick phone, a fax, a '
         + 'pager, a video tape. A current object makes it an advert. And the '
         + 'stamp line has to be an order in two or three words — a sentence in '
         + 'there reads as a heading and the sheet loses its shape.',

    slots: [
      { id: 'frame', t: 'frame', box: [0.129, 0.038, 0.734, 0.132],
        fill: 'accent', r: 0.022, weight: 0.005 },
      { id: 'stamp', t: 'type', box: [0.145, 0.054, 0.693, 0.100],
        role: 'black', weight: '900', fill: 'accent', align: 'center', track: -0.01,
        fit: 'shrink', size: 0.124, min: 0.060,
        wants: 'An order, in caps, two or three words, ending in an exclamation '
             + 'mark. What the world keeps telling you to do.' },

      { id: 'burst', t: 'burst', box: [0.118, 0.197, 0.810, 0.608],
        fill: 'accent', points: 15, inner: 0.40, jitter: 0.22 },
      { id: 'object', t: 'art', role: 'object', box: [0.130, 0.245, 0.770, 0.530],
        treat: 'contain', cut: true, shapeTol: 1.2, tint: 'print', polarity: 'keep',
        wants: 'One obsolete object, cut out, lying diagonally across the burst: '
             + 'a brick phone, a fax machine, a pager, a cassette. It is the thing '
             + 'the order was invented for, and it has stopped working.' },

      { id: 'hole', t: 'burst', box: [0.555, 0.200, 0.330, 0.240],
        fill: 'ground', points: 7, inner: 0.46, jitter: 0.20 },
      { id: 'callout', t: 'burst', box: [0.615, 0.285, 0.200, 0.115],
        fill: 'accent', points: 5, inner: 0.44, jitter: 0.30,
        outline: true, weight: 0.022 },
      { id: 'cry', t: 'type', box: [0.630, 0.318, 0.170, 0.050],
        role: 'marker', weight: '400', fill: 'accent', align: 'center',
        fit: 'shrink', size: 0.048, min: 0.026,
        wants: 'ONE word inside the callout. What the object is saying.' },

      { id: 'lineA', t: 'type', box: [0.042, 0.826, 0.512, 0.068],
        role: 'marker', weight: '400', fill: 'accent',
        fit: 'shrink', size: 0.077, min: 0.040,
        wants: 'The first half of the turn, in felt-tip, hard against the left '
             + 'margin. Lower case. It should sound like somebody muttering.' },
      { id: 'lineB', t: 'type', box: [0.348, 0.904, 0.569, 0.058],
        role: 'marker', weight: '400', fill: 'accent',
        fit: 'shrink', size: 0.063, min: 0.034,
        wants: 'The second half, stepped right and slightly smaller, trailing off '
             + 'into dots. This is the line that answers the stamp.' },
    ],
  },
  ...
  /* ------------------------------------------------------------- h027
   * Measured off .refs/hooks/h027.jpg (736x920, 0.80) with tighten.py:
   *   handle .048/.060 .122x.021
   *   head   .049/.274 .753x.215, two lines
   *     line one .054/.274 .747x.096, line two .049/.395 .708x.095
   *   so cap height .096, pitch .121: size .133 and leading .91, which
   *   is the descender of one line sitting inside the next line's
   *   shoulder. That tightness is the sheet.
   * The ground is the whole design. GROUNDS.red is this exact pairing:
   * an orange red flood with near-black type on it.
   */
  h027: {
    id: 'h027', family: 'field', ground: 'red', ref: 'h027', src: 0.80,

    does: 'A flood of one colour, one instruction across it in heavy lowercase, '
        + 'and nothing else on the sheet at all. No picture, no rule, no second '
        + 'thought. It is the plainest thing in the set and the hardest to use, '
        + 'because with nothing to look at, the only thing carrying it is whether '
        + 'the sentence is worth saying that loudly. For an instruction you would '
        + 'stand behind in an argument.',
    avoid: 'Anything hedged. "Consider", "maybe", "it might be worth" — a flood '
         + 'of colour behind a suggestion looks like shouting at nobody. It also '
         + 'cannot carry an explanation: there is one line and no room to '
         + 'qualify it, so a sentence that needs a second sentence needs a '
         + 'different sheet. And the language does the work here, so it is the '
         + 'one place a cheap intensifier shows. The reference reaches for a '
         + 'swear word; the sheet works without one and looks worse with one.',

    slots: [
      { id: 'handle', t: 'type', box: [0.048, 0.055, 0.300, 0.028],
        role: 'grotesque', weight: '700', fill: 'mark', align: 'left',
        fit: 'shrink', size: 0.024, min: 0.016,
        wants: 'The handle, top left, small. It is the only other ink on the '
             + 'sheet, so it reads as a signature on the instruction rather than '
             + 'as branding.' },

      { id: 'head', t: 'type', box: [0.049, 0.274, 0.800, 0.215],
        role: 'display', weight: '800', fill: 'mark', align: 'left',
        fit: 'shrink', size: 0.133, min: 0.075, leading: 0.91,
        wants: 'The instruction. Sentence case, not caps: this is said, not '
             + 'announced. TWO lines, four or five words in total, and it ends '
             + 'in a full stop, which is what stops it reading as a slogan. The '
             + 'two lines want to be close to the same width, the second a '
             + 'little shorter: the block is the only shape on the sheet, and a '
             + 'short second line leaves a hole in it.' },
    ],
  },
  ...
  /* ------------------------------------------------------------- h035
   * Measured off .refs/hooks/h035.jpg (735x920, 0.80) with tighten.py.
   * Seven runs of cells, and the cell size is the same in every one of
   * them, which is what makes it read as one grid rather than seven
   * labels:
   *   WHY      .150/.139 .262   NOBODYS  .299/.222 .612
   *   OBSESSED .070/.468 .699   WITH     .243/.601 .350
   *   YOUR     .460/.654 .350   BRAND    .158/.725 .437
   *   YET      .539/.869 .262
   * Divided by their letter counts that is .0873 .0874 .0874 .0875
   * .0875 .0874 .0873 — one cell, .0874 wide and .086 tall. Slightly
   * taller than wide, which is the reference's own proportion.
   * The four rails: .075/.025 .264x.011   .740/.025 .185x.011
   *                 .075/.961 .186x.012   .661/.961 .264x.012
   */
  h035: {
    id: 'h035', family: 'grid-tiles', ground: 'paper', ref: 'h035', src: 0.80,

    does: 'A question laid into crossword cells over a photograph, one letter to '
        + 'a cell, the runs stepping down and across the sheet rather than '
        + 'stacking. The cells are the point: letters in boxes cannot be taken '
        + 'in at a glance, so the eye has to walk them, and walking them is what '
        + 'stops a scroll. Four small rails at the corners hold the handle. For '
        + 'a question you want somebody to finish reading before they decide '
        + 'whether it is about them.',
    avoid: 'Anything long. Every letter costs a cell and the sheet holds about '
         + 'thirty of them, so a question over six words runs out of grid. It '
         + 'also cannot take a word people scan rather than read — a URL, a '
         + 'price, a date — because spelling those out in boxes is a joke at the '
         + 'reader\'s expense. And the photograph has to survive being covered: '
         + 'the runs sit across the middle of it, so a picture whose subject is '
         + 'in the middle loses it.',

    slots: [
      /* Ten columns by thirteen rows of square cells, measured off the
         reference's own ruling: the picture is 629 x 817 px on a 735 x
         920 sheet, and both divisions come out at a 62.9 px pitch. Every
         run below is stated as the cells it fills rather than as a box
         somebody drew round it, which is why they line up. */
      { id: 'scene', t: 'art', role: 'scene', box: [0.0721, 0.0554, 0.8558, 0.8880],
        treat: 'cover', polarity: 'keep',
        wants: 'A flat overhead or straight-on shot of a surface with a few '
             + 'objects on it: a table, a bench, a board. Strongly one colour, '
             + 'so the cells read against it, and with its interest around the '
             + 'edges rather than in the middle, because the runs cross the '
             + 'centre. Something being played or made, caught mid-action.' },

      /* The empty grid, ruled across the whole picture at the same pitch
         as the runs. Without it the tiles read as scattered labels; with
         it they read as a crossword, because the ruling says the cells
         were always there and some have been filled in. */
      { id: 'grid', t: 'grid', box: [0.0721, 0.0554, 0.8558, 0.8880],
        cell: [0.0856, 0.0683], fill: 'mark', weight: 0.0022, alpha: 0.55 },

      /* Cells, not boxes: [column, row, length] read off the grid, then
         turned into the frame's coordinates by the same arithmetic that
         drew it. Nothing here was nudged by eye. */
      { id: 'runA', t: 'tiles', box: [0.1577, 0.1237, 0.2567, 0.0683],
        cell: [0.0856, 0.0683], align: 'left',
        fill: 'ground', on: 'mark',
        wants: 'Word 1 of the question, one letter to a cell. Three letters in the '
             + 'reference; it grows rightward from column two, so up to about five '
             + 'still sits clear of the run beneath it.' },
      { id: 'runB', t: 'tiles', box: [0.3288, 0.1921, 0.5990, 0.0683],
        cell: [0.0856, 0.0683], align: 'right',
        fill: 'ground', on: 'mark',
        wants: 'Word 2. Seven letters in the reference. It is nailed to the right '
             + 'edge of the grid and grows leftward, so a longer word runs back '
             + 'under the first rather than off the sheet.' },
      { id: 'runC', t: 'tiles', box: [0.0721, 0.3970, 0.6846, 0.0683],
        cell: [0.0856, 0.0683], align: 'left',
        fill: 'ground', on: 'mark',
        wants: 'Word 3, and the widest run: it starts hard against the left edge '
             + 'of the grid. Eight letters in the reference, ten before it runs '
             + 'out.' },
      { id: 'runD', t: 'tiles', box: [0.2433, 0.6019, 0.3423, 0.0683],
        cell: [0.0856, 0.0683], align: 'left',
        fill: 'ground', on: 'mark',
        wants: 'Word 4. Four letters, growing rightward from column three. It and '
             + 'the run below it are the pair that step across the middle, so '
             + 'keeping them short is what keeps the stagger legible.' },
      { id: 'runE', t: 'tiles', box: [0.5000, 0.6702, 0.3423, 0.0683],
        cell: [0.0856, 0.0683], align: 'right',
        fill: 'ground', on: 'mark',
        wants: 'Word 5. Four letters, nailed to column nine and growing leftward.' },
      { id: 'runF', t: 'tiles', box: [0.1577, 0.7385, 0.4279, 0.0683],
        cell: [0.0856, 0.0683], align: 'left',
        fill: 'ground', on: 'mark',
        wants: 'Word 6. Five letters, growing rightward from column two.' },
      { id: 'runG', t: 'tiles', box: [0.5856, 0.8752, 0.2567, 0.0683],
        cell: [0.0856, 0.0683], align: 'right',
        fill: 'ground', on: 'mark',
        wants: 'The last word, nailed to column nine and growing leftward. Three '
             + 'letters, and it wants to be the one that turns the question: it '
             + 'sits alone at the foot with two empty rows above it.' },

      { id: 'railTL', t: 'type', box: [0.0721, 0.0241, 0.3000, 0.016],
        role: 'grotesque', weight: '500', fill: 'mark', align: 'left', track: 0.04,
        fit: 'shrink', size: 0.0153, min: 0.010,
        wants: 'What you do, in three or four words, caps. Not the handle.' },
      { id: 'railTR', t: 'type', box: [0.6279, 0.0241, 0.3000, 0.016],
        role: 'grotesque', weight: '500', fill: 'mark', align: 'right', track: 0.04,
        fit: 'shrink', size: 0.0153, min: 0.010,
        wants: 'The handle, caps.' },
      { id: 'railBL', t: 'type', box: [0.0721, 0.9601, 0.3000, 0.016],
        role: 'grotesque', weight: '500', fill: 'mark', align: 'left', track: 0.04,
        fit: 'shrink', size: 0.0153, min: 0.010,
        wants: 'The handle again. The four rails repeat two lines diagonally, so '
             + 'this one matches the top right.' },
      { id: 'railBR', t: 'type', box: [0.6279, 0.9601, 0.3000, 0.016],
        role: 'grotesque', weight: '500', fill: 'mark', align: 'right', track: 0.04,
        fit: 'shrink', size: 0.0153, min: 0.010,
        wants: 'And this one matches the top left.' },
    ],
  },
   ...
   */
  /* ------------------------------------------------------------- h009
   * Rebuilt on the desk illustration Ashley supplied, measured off it at
   * 1086x1448: six bars, x 0.2486 to 0.8135, and they are NOT parallel.
   * The tilt runs +0.51 degrees at the top to +2.00 at the bottom and the
   * bars shorten from 0.0541 of the frame to 0.0488, which is a screen
   * seen at an angle. Parallel bars on the same picture read as a caption
   * pasted over a photograph rather than as words on a monitor, so the
   * bend is the whole device and it is carried as a progression.
   *
   * The `bars` slot sizes itself: it shrinks one size until the longest
   * line fits the slab and reports if even the minimum does not. So an
   * agent cannot write words out of the side of a monitor, which is the
   * one failure this layout could produce and not survive.
   */
  h009: {
    id: 'h009', family: 'screen', ground: 'ink', ref: 'h009',

    does: 'Six short lines on the screen of a desk machine, following the '
        + 'bend of it. For a list where every item is the same KIND of thing '
        + 'and the reader is meant to see the length of the list before they '
        + 'read any of it.',
    avoid: 'Not for a sentence broken across six bars. Each line has to stand '
         + 'alone, because the bend and the slabs make them read as six things '
         + 'and not as one paragraph.',

    slots: [
      { id: 'scene', t: 'art', role: 'scene', box: [0, 0, 1, 1],
        scene: 'desk', treat: 'cover', pin: false,
        wants: 'The desk illustration. It is the layout, not a background: the '
             + 'bars are measured against this frame and nothing else fits '
             + 'them.' },

      /* No `bar`: the white slabs are in the illustration already, and
         the type is set on them in black. `fill` is a literal rather
         than a ground name for the same reason — the ink here belongs to
         the picture, not to the sheet's palette. */
      { id: 'screen', t: 'bars', box: [0.2486, 0, 0.5649, 0.42],
        role: 'grotesque', weight: '700', fill: '#14120F',
        /* Measured in IMAGE fractions and converted to SHEET ones: the
           illustration is 1086x1448 and the sheet is 1080x1350, so cover
           scales it 0.9945 and centres 1440px of picture in 1350px of
           frame. y_sheet = y_image * 1.0667 - 0.0333, and every vertical
           number here has been through that. Without it the bars sit
           0.022 of the frame below the screen they belong to. */
        lines: 6, y0: 0.1528, pitch: 0.07499, dpitch: -0.00043,
        slope0: 0.00953, dslope: 0.02766,
        barH: 0.05771, dbarH: -0.00565,
        size: 0.030, min: 0.017, padX: 0.018, track: -0.03,
        wants: 'Up to six lines, each one a whole thought. About 26 characters '
             + 'a line at the set size; longer and every line shrinks together '
             + 'to keep them one size, so one long line costs all six. Write '
             + 'them the same grammatical shape: they are read as a column.' },
    ],
  },

  /* ------------------------------------------------------------- h015
   * Measured off .refs/hooks/h015.jpg (736x981, 0.75) with tighten.py:
   *   mark  .473/.035 .057x.042
   *   lineA .262/.303 .447x.047   (didone italic, ascender to descender)
   *   lineB .072/.397 .833x.077   (black caps, cap height .077)
   *   lineC .091/.484 .778x.126   (didone italic, cap to baseline)
   *   footL .050/.944 .164x.017   tag .425/.944 .020x.017
   *   footC .455/.944 .127x.017   footR .845/.945 .137x.021
   * The three headline layers overlap on purpose: lineC's capital rises
   * into lineB's baseline. So they are three boxes that touch, not a
   * stack, and lineB draws before lineC.
   */
  h015: {
    id: 'h015', family: 'photo-field', ground: 'paper', ref: 'h015', src: 0.75,

    does: 'A grainy photograph — one body in the air over water, nothing else in '
        + 'the frame — with a three-layer headline printed across its middle. A '
        + 'small italic serif sets the question, one word in black caps runs the '
        + 'full width under it, and then the answer comes back in a huge italic '
        + 'serif in the accent colour, riding up into the caps so the two overlap. '
        + 'A drawn asterisk sits at the top centre and a save row runs along the '
        + 'foot. It is a saveable card: the promise is that the swipe is worth '
        + 'keeping.',
    avoid: 'The middle word is set in caps at four fifths of the width, so it has '
         + 'to be one word of about seven letters — a longer one shrinks and the '
         + 'whole sheet loses its spine. And the picture has to be one figure with '
         + 'space around it: the type sits in the middle third, so a busy '
         + 'photograph puts a serif italic over somebody\'s face.',

    slots: [
      { id: 'scene', t: 'art', role: 'scene', box: [0.000, 0.000, 1.000, 1.000],
        tint: 'ground', key: 0.30, contrast: 0.92, polarity: 'keep',
        wants: 'One figure in the air, or falling, or alone in a wide flat frame, '
             + 'shot grainy and high-key. The top half has to be near-empty — it '
             + 'is where the headline lands — and the subject belongs in the '
             + 'bottom third, under the type.' },

      { id: 'mark', t: 'star', box: [0.473, 0.035, 0.057, 0.042],
        fill: 'mark', count: 8, weight: 0.22, turn: 11 },

      { id: 'lineA', t: 'type', box: [0.262, 0.300, 0.447, 0.050],
        role: 'didoneItalic', weight: '500', fill: 'mark', align: 'center',
        fit: 'shrink', size: 0.049, min: 0.028,
        wants: 'The question, in three or four words, as an italic serif. It sets '
             + 'up the word underneath and nothing else — no claim of its own.' },
      { id: 'lineB', t: 'type', box: [0.072, 0.390, 0.833, 0.086],
        role: 'black', weight: '900', fill: 'mark', align: 'center', track: -0.02,
        fit: 'shrink', size: 0.105, min: 0.055,
        wants: 'ONE word in caps, filling the width. The subject. Seven letters is '
             + 'the reference; ten is the most that will hold the size.' },
      { id: 'lineC', t: 'type', box: [0.091, 0.470, 0.778, 0.145],
        role: 'didoneItalic', weight: '500', fill: 'accent', align: 'center',
        fit: 'shrink', size: 0.180, min: 0.090,
        wants: 'ONE word in the accent colour, the biggest thing on the sheet, '
             + 'riding up into the caps above it. The answer — what the subject '
             + 'is actually about.' },

      { id: 'footL', t: 'type', box: [0.050, 0.942, 0.220, 0.020],
        role: 'didoneItalic', weight: '500', fill: 'mark',
        fit: 'shrink', size: 0.019, min: 0.012,
        wants: 'The handle, bottom left, in the same italic serif.' },
      { id: 'tag', t: 'badge', box: [0.425, 0.942, 0.020, 0.020],
        icon: 'bookmark', fill: 'mark', ring: false },
      { id: 'footC', t: 'type', box: [0.455, 0.942, 0.180, 0.020],
        role: 'didoneItalic', weight: '500', fill: 'mark',
        fit: 'shrink', size: 0.019, min: 0.012,
        wants: 'Three words beside the tag. What to do with this: save it, keep '
             + 'it, come back to it.' },
      { id: 'footR', t: 'arrow', box: [0.845, 0.945, 0.137, 0.021],
        fill: 'mark', weight: 0.16 },
    ],
  },


  /* ------------------------------------------------------------- h016
   * Measured off .refs/hooks/h016.jpg (735x920, 0.80), the headline read
   * off a 1.6x crop because the sky gradient defeats a border median:
   *   baselines .140 / .213 / .284 / .355, pitch .0715, cap height .0598
   *   so size .083 and leading 0.86 — the lines nearly touch, which is
   *   the whole look. Widest line .204-.791, block centred on .494.
   *   globe  .193/.074 .067x.076    spark  .644/.070 .179x.082
   *   rings  .138/.291 .107x.065    faces  .734/.301 .148x.055
   *   corner sparkles .043/.028 .037x.029 and mirrored on all four
   *   pill   .359/.940 .286x.024
   */
  h016: {
    id: 'h016', family: 'collage', ground: 'blue', ref: 'h016', src: 0.80,

    does: 'A staged collage — blue sky, green hill, and one absurd tableau in the '
        + 'middle of it — with four lines of fat rounded caps stacked so tightly '
        + 'across the top that they almost touch. Hairline outline ornaments hang '
        + 'off the corners of the block: a wireframe globe, a long thin sparkle, '
        + 'three overlapping rings, three faces changing their mind. Four more '
        + 'sparkles sit in the four corners of the sheet and a pill at the foot '
        + 'holds the handle. It is a complaint told as a joke, and the picture '
        + 'has to do the complaining.',
    avoid: 'The sentence has to break into four short lines of roughly equal '
         + 'width — it is set as a solid block and a long line in the middle '
         + 'shrinks the whole thing. And the photograph has to be genuinely '
         + 'absurd: staged, over-lit, one impossible thing happening calmly. A '
         + 'normal stock photo under this type reads as a caption on a slide.',

    slots: [
      { id: 'scene', t: 'art', role: 'scene', box: [0.000, 0.000, 1.000, 1.000],
        polarity: 'keep',
        wants: 'A staged, over-lit, faintly impossible scene: somebody doing the '
             + 'wrong thing in the wrong place, calmly, in the open air. Bright '
             + 'and saturated, with the top third empty enough to take four lines '
             + 'of heavy type.' },

      { id: 'head', t: 'type', over: 'art', polarity: 'keep',
        box: [0.201, 0.075, 0.590, 0.281],
        role: 'fat', weight: '400', fill: 'mark', align: 'center',
        fit: 'shrink', size: 0.083, min: 0.045, leading: 0.86,
        wants: 'The whole sentence in caps, broken into FOUR lines. Lines two and '
             + 'three are the widest; the FIRST and LAST have to be short, because '
             + 'the ornaments sit in the gaps beside them and a long last line '
             + 'runs straight through the rings. It is a reaction, not a '
             + 'statement — the thing you would actually say out loud after it '
             + 'happened. Trailing dots do more here than an exclamation mark.' },

      { id: 'globe', t: 'globe', box: [0.193, 0.074, 0.067, 0.076], fill: 'mark' },
      { id: 'spark', t: 'sparkle', box: [0.644, 0.070, 0.179, 0.082],
        fill: 'mark', outline: true, pinch: 0.10, weight: 0.030 },
      { id: 'rings', t: 'rings', box: [0.138, 0.291, 0.107, 0.065],
        fill: 'mark', count: 3, overlap: 0.46, weight: 0.09 },
      { id: 'faces', t: 'rings', box: [0.734, 0.301, 0.148, 0.055],
        fill: 'mark', count: 3, overlap: 0.30, weight: 0.10, face: true },

      { id: 'cornerTL', t: 'sparkle', box: [0.043, 0.028, 0.037, 0.029], fill: 'mark' },
      { id: 'cornerTR', t: 'sparkle', box: [0.920, 0.028, 0.037, 0.029], fill: 'mark' },
      { id: 'cornerBL', t: 'sparkle', box: [0.043, 0.938, 0.037, 0.029], fill: 'mark' },
      { id: 'cornerBR', t: 'sparkle', box: [0.920, 0.938, 0.037, 0.029], fill: 'mark' },

      { id: 'pill', t: 'pill', over: 'art', polarity: 'keep', box: [0.359, 0.936, 0.286, 0.030],
        fill: 'mark', weight: 0.07 },
      { id: 'handle', t: 'type', over: 'art', polarity: 'keep', box: [0.375, 0.941, 0.254, 0.020],
        role: 'grotesque', weight: '700', fill: 'mark', align: 'center', track: 0.03,
        fit: 'shrink', size: 0.017, min: 0.011,
        wants: 'The handle, in caps, inside the pill.' },
    ],
  },


  /* ------------------------------------------------------------- h021
   * Measured off .refs/hooks/h021.jpg (736x868, 0.80) with tighten.py.
   * Three scales, and the jump between them is the whole device:
   *   left  .231/.237 .041x.011     right .720/.237 .038x.011
   *   over  .462/.366 .075x.015     under .482/.566 .037x.016
   *   word  .248/.405 .506x.107     — ten times the cap height of the
   *   rest, read off a 1.6x crop because the lip highlight defeats a
   *   border median on the full sheet.
   * The run at the foot is two lines, baselines .948 and .971, cap
   * height .011, spanning .015 to the right edge in broken pieces.
   */
  h021: {
    id: 'h021', family: 'halftone-portrait', ground: 'ink', ref: 'h021', src: 0.80,

    does: 'One short sentence taken apart and set at three scales over an extreme '
        + 'close crop, so the eye assembles it rather than reads it. A pair of '
        + 'words sits across the top, wide apart; under them the sentence runs '
        + 'down the middle in three pieces with the middle one enormous. Nothing '
        + 'is centred as a block and nothing lines up: the scale jump does the '
        + 'work a layout usually does. For a line that is worth slowing down, '
        + 'where the pause between the pieces changes what it means.',
    avoid: 'The sentence has to be three or four words. This breaks a phrase, not '
         + 'a statement, and a middle word longer than about seven characters '
         + 'stops being enormous and becomes merely large, which is the one thing '
         + 'this cannot survive. The photograph has to be a crop close enough to '
         + 'be abstract: a whole face or a recognisable scene competes with the '
         + 'type instead of holding it.',

    slots: [
      { id: 'scene', t: 'art', role: 'scene', box: [0.000, 0.000, 1.000, 1.000],
        polarity: 'keep',
        wants: 'An extreme close crop, filling the frame, of something warm and '
             + 'soft enough to read as a ground rather than a subject: skin, '
             + 'fruit, cloth, paint. Cropped past the point of being '
             + 'identifiable — no whole face, no scene with an edge in it. It '
             + 'wants one bright incident somewhere off centre for the type to '
             + 'sit against, and grain rather than polish. The foot of the frame '
             + 'has to fall away dark: the run sits on it at half strength, and '
             + 'over a bright bottom edge it disappears entirely rather than '
             + 'reading as faint.' },

      { id: 'left', t: 'type', over: 'art', polarity: 'keep',
        box: [0.232, 0.2373, 0.130, 0.014],
        role: 'grotesque', weight: '700', fill: 'mark', align: 'left', track: 0.06,
        fit: 'shrink', size: 0.0125, min: 0.009,
        wants: 'One word, caps. It pairs with the word on the right: a rhyme, or '
             + 'the same word one letter different, or a before to its after. On '
             + 'its own it should not resolve.' },

      { id: 'right', t: 'type', over: 'art', polarity: 'keep',
        box: [0.625, 0.2373, 0.130, 0.014],
        role: 'grotesque', weight: '700', fill: 'mark', align: 'right', track: 0.06,
        fit: 'shrink', size: 0.0125, min: 0.009,
        wants: 'Its pair. Same length as the word on the left if you can manage '
             + 'it, because the gap between them is what makes them read as a '
             + 'pair rather than as two labels.' },

      { id: 'over', t: 'type', over: 'art', polarity: 'keep',
        box: [0.3485, 0.3648, 0.300, 0.022],
        role: 'grotesque', weight: '700', fill: 'mark', align: 'center', track: 0.04,
        fit: 'shrink', size: 0.0208, min: 0.014,
        wants: 'The first piece of the sentence. One or two words, caps. It is a '
             + 'run-up, so it should be the part that means least on its own.' },

      { id: 'word', t: 'type', over: 'art', polarity: 'keep',
        box: [0.0705, 0.4089, 0.860, 0.105],
        role: 'black', weight: '900', fill: 'mark', align: 'center',
        fit: 'shrink', size: 0.1347, min: 0.090,
        wants: 'The middle piece, and the reason the sheet exists. ONE word, caps, '
             + 'seven characters at the outside. Ending it with a full stop is '
             + 'what makes the pieces above and below read as separate breaths '
             + 'rather than as a sentence somebody has spaced out.' },

      { id: 'under', t: 'type', over: 'art', polarity: 'keep',
        box: [0.349, 0.5658, 0.300, 0.022],
        role: 'grotesque', weight: '700', fill: 'mark', align: 'center', track: 0.04,
        fit: 'shrink', size: 0.0194, min: 0.013,
        wants: 'The last piece. One or two words, caps, and it has to be the turn: '
             + 'the part that makes the enormous word above it mean something '
             + 'other than what it meant on its own.' },

      /* Two lines given, not one wrapped. The reference's run is broken
         into pieces with gaps between them, and a wrap would close those
         gaps up into a paragraph, which is the one thing it is not. */
      { id: 'run', t: 'type', over: 'art', polarity: 'keep',
        box: [0.000, 0.9465, 1.000, 0.048],
        role: 'grotesque', weight: '700', fill: 'mark', align: 'left',
        fit: 'shrink', size: 0.0184, min: 0.012, leading: 1.315, alpha: 0.62,
        wants: 'TWO lines, given as two, of fragments rather than sentences: '
             + 'overheard pieces in the language somebody would actually use '
             + 'about the subject, with runs of spaces between them so they sit '
             + 'apart across the width. It is texture, not information, set '
             + 'faint enough to be found rather than read, and nothing in it '
             + 'should be needed to understand the sheet.' },
    ],
  },
  ...
  h006: {
    ...
    slots: [
      ...
      /* The white shirt, pinned. It is cut on the bottom alone, so it
         stands on a horizontal: here that is the panel's own foot, and
         the box ends there. Left and mirrored so he faces into the
         sheet rather than out of it. */
      /* Stands on the STRIP RULE at 0.782, not on the panel's foot: the
         rule is the horizontal this sheet actually has, and standing him
         on it puts him across the panel rather than inside it. Unflipped,
         so he faces right, into the sheet. */
      { id: 'plate', t: 'art', own: 'sky-arms', cut: true,
        /* Runs off the left edge on purpose. His foot stays on 0.782,
           which is the strip rule, so the box grows UPWARD from there:
           0.782 - 0.540 = 0.242. Width follows his own shape, 0.81 wide
           over tall, or the cell would letterbox him. */
        box: [-0.060, 0.242, 0.547, 0.540],
  ...
  /* ------------------------------------------------------------- h011
   * Nine lines, each measured with tools/tighten.py:
   *   .064/.107  .087/.168  .296/.261  .477/.328  .079/.412
   *   .076/.493  .076/.568  .397/.691  .080/.737
   *   foot rail .236/.918 .474x.026
   */
  h011: {
    id: 'h011', family: 'type-as-image', ground: 'blue', ref: 'h011',

    does: 'One sentence in nine short lines of heavy caps that step across a flat '
        + 'field of colour, each line starting where the eye left the last. The '
        + 'edges are sprayed rather than printed. There is no picture, no second '
        + 'voice and nothing else at all: the stepping is what makes it read as a '
        + 'chant instead of a paragraph.',
    avoid: 'Every line has to be a phrase you would pause after. Breaking a '
         + 'sentence at a preposition to make the steps work ruins it, because the '
         + 'reader pauses where you broke it whether you meant them to or not. '
         + 'One or two words a line, never three.',

    slots: [
      { id: 'l1', t: 'type', box: [0.064, 0.107, 0.540, 0.078],
        role: 'display', weight: '800', fill: 'mark', track: -0.02, soft: 0.030,
        fit: 'shrink', size: 0.100, min: 0.045, vAlign: 'bottom',
        wants: 'The first phrase, flush left at the top.' },
      { id: 'l2', t: 'type', box: [0.087, 0.168, 0.330, 0.100],
        role: 'display', weight: '800', fill: 'mark', track: -0.02, soft: 0.030,
        fit: 'shrink', size: 0.128, min: 0.045, vAlign: 'bottom',
        wants: 'The second, indented slightly.' },
      { id: 'l3', t: 'type', box: [0.296, 0.261, 0.645, 0.091],
        role: 'display', weight: '800', fill: 'mark', track: -0.02, soft: 0.030,
        fit: 'shrink', size: 0.116, min: 0.045, vAlign: 'bottom',
        wants: 'The third, stepped a long way right.' },
      { id: 'l4', t: 'type', box: [0.477, 0.328, 0.460, 0.090],
        role: 'display', weight: '800', fill: 'mark', track: -0.02, soft: 0.030,
        fit: 'shrink', size: 0.115, min: 0.045, vAlign: 'bottom',
        wants: 'The fourth, further right again — the far side of the sheet.' },
      { id: 'l5', t: 'type', box: [0.079, 0.412, 0.520, 0.100],
        role: 'display', weight: '800', fill: 'mark', track: -0.02, soft: 0.030,
        fit: 'shrink', size: 0.128, min: 0.045, vAlign: 'bottom',
        wants: 'The fifth, back to the left margin. The turn.' },
      { id: 'l6', t: 'type', box: [0.076, 0.493, 0.360, 0.100],
        role: 'display', weight: '800', fill: 'mark', track: -0.02, soft: 0.030,
        fit: 'shrink', size: 0.128, min: 0.045, vAlign: 'bottom',
        wants: 'The sixth.' },
      { id: 'l7', t: 'type', box: [0.076, 0.568, 0.310, 0.100],
        role: 'display', weight: '800', fill: 'mark', track: -0.02, soft: 0.030,
        fit: 'shrink', size: 0.128, min: 0.045, vAlign: 'bottom',
        wants: 'The seventh, the shortest.' },
      { id: 'l8', t: 'type', box: [0.397, 0.691, 0.552, 0.069],
        role: 'display', weight: '800', fill: 'mark', track: -0.02, soft: 0.030,
        fit: 'shrink', size: 0.088, min: 0.045, vAlign: 'bottom',
        wants: 'The eighth, stepped right again.' },
      { id: 'l9', t: 'type', box: [0.080, 0.737, 0.836, 0.100],
        role: 'display', weight: '800', fill: 'mark', track: -0.02, soft: 0.030,
        fit: 'shrink', size: 0.128, min: 0.045, vAlign: 'bottom',
        wants: 'The last phrase, widest, running the full width.' },
      { id: 'foot', t: 'type', box: [0.236, 0.945, 0.474, 0.034],
        role: 'grotesque', weight: '700', fill: 'mark', align: 'center', track: 0.08,
        fit: 'shrink', size: 0.022, min: 0.012, vAlign: 'middle',
        wants: 'One line at the foot: a date, a mark and a handle, spaced apart.' },
    ],
  },
  ...
  h028: {
    ...
    slots: [
      ...
      /* One icon, in a gap the layout actually has. Chosen for what the
         sheet SAYS, and the copy may name a different one: the position
         is the design and the object is the argument. */
      { id: 'icon', t: 'art', icon: 'sparkle-gold', box: [0.862, 0.300, 0.100, 0.070],
        treat: 'contain',
        wants: 'One object from the pack, for what it means here. Change the '
             + 'name if a different one fits the words better.' },
    ],
  ...
  /* ------------------------------------------------------------- h012
   * Measured with tools/tighten.py on .refs/hooks/h012.jpg (736x920, 0.80):
   *   dial   .061/.051 .067x.053
   *   head   .149/.194 .702x.116  — two lines, line 1 ink .202/.194 .594x.039,
   *          line 2 .149/.262 .702x.048, so one size, pitch .068 → leading 1.26
   *   sub    .257/.330 .484x.015
   *   scene  cream still shows at x<.02 and x>.98 down to y .71, so the picture
   *          is a cut-out standing on the bottom edge, not a full-bleed crop.
   *          Subject top .465, runs off the bottom and both sides.
   *   footL  .073/.921 .177x.010   footC  .391/.904 .245x.030
   *   footR  three lines .620/.902, .620/.925, .630/.948, right edge .979
   * No band under the foot — the picture is simply dark there and the credit
   * is knocked out of it in paper.
   */
  h012: {
    id: 'h012', family: 'ui-device', ground: 'paper', ref: 'h012', src: 0.80,

    does: 'A two-line claim centred across the top, one quiet line under it, and a '
        + 'person seen from behind facing a screen that has gone wrong. The reader '
        + 'is over their shoulder, which is the whole device: you are not being '
        + 'shown a problem, you are watching somebody have it. A three-part credit '
        + 'runs along the foot in paper, over the dark of the picture.',
    avoid: 'The screen has to be legibly wrong — dialogs, errors, something '
         + 'stacking up. A generic desk photograph puts the reader over the '
         + 'shoulder of somebody doing nothing, and the claim then has no evidence '
         + 'at all. The credit needs the picture to be dark under it; if the '
         + 'bottom of the picture is light, the foot disappears.',

    slots: [
      { id: 'dial', t: 'dial', box: [0.061, 0.051, 0.067, 0.053],
        fill: 'mark', from: 20, to: 90, weight: 0.20, hub: 0.18 },

      { id: 'head', t: 'type', box: [0.149, 0.190, 0.702, 0.125],
        role: 'geometric', weight: '600', fill: 'mark', align: 'center', track: -0.005,
        fit: 'wrap', size: 0.054, min: 0.034, leading: 1.26, vAlign: 'top',
        wants: 'The claim, in about six or seven words, breaking over two lines. '
             + 'It names a cost — what the confusion, the wait, the silence is '
             + 'charging. Not a feeling and not an instruction.' },
      { id: 'sub', t: 'type', box: [0.257, 0.326, 0.484, 0.024],
        role: 'grotesque', weight: '500', fill: 'mark', align: 'center',
        fit: 'shrink', size: 0.021, min: 0.014, vAlign: 'top',
        wants: 'One quiet line under it, six or seven words. What to do about it.' },

      { id: 'scene', t: 'art', role: 'figure', box: [0.000, 0.455, 1.000, 0.545],
        treat: 'contain', anchor: 'bottom', shapeTol: 1.4, cut: true,
        wants: 'A person seen from behind at a screen, cut out of its background, '
             + 'standing on the bottom edge so it runs off the sheet. The screen '
             + 'has to be legible enough to read as going wrong, and the back of '
             + 'the head matters: it is what puts the reader in the room rather '
             + 'than in front of a poster.' },

      { id: 'footL', t: 'type', box: [0.073, 0.858, 0.177, 0.020],
        role: 'grotesque', weight: '600', fill: 'ground', track: 0.06,
        fit: 'shrink', size: 0.014, min: 0.010, vAlign: 'top',
        wants: 'Four words, caps, bottom left. "brought to you by".' },
      { id: 'footC', t: 'type', box: [0.375, 0.852, 0.280, 0.026],
        role: 'grotesque', weight: '800', fill: 'ground', align: 'center', track: 0.05,
        fit: 'shrink', size: 0.020, min: 0.013, vAlign: 'top',
        wants: 'A name, centred, heavier than the rest.' },
      { id: 'footR', t: 'type', box: [0.620, 0.828, 0.359, 0.078],
        role: 'grotesque', weight: '600', fill: 'ground', align: 'right', track: 0.03,
        fit: 'wrap', size: 0.019, min: 0.012, leading: 1.22, vAlign: 'top',
        wants: 'Three short lines of caps, right. What you do, listed — the '
             + 'longest one last, so the block reads as a wedge.' },
      /* One icon, in a gap the layout actually has. Chosen for what the
         sheet SAYS, and the copy may name a different one: the position
         is the design and the object is the argument. */
      { id: 'icon', t: 'art', icon: 'no', box: [0.815, 0.055, 0.110, 0.075],
        treat: 'contain',
        wants: 'One object from the pack, for what it means here. Change the '
             + 'name if a different one fits the words better.' },
    ],
