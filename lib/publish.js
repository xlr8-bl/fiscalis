 ...
 *
 * @param opts.visibility 'test' posts at TikTok's SELF_ONLY, so it
 *   really goes to TikTok and only he can see it. Instagram has no
 *   private post at all, so it is skipped with that as the reason
 *   rather than quietly posted in public.
...
export async function postOne(env, slug, opts = {}) {
  ...
  /* Instagram has no private post. There is no API for one and there is
     no such thing in the app either: a post is visible to whoever can
     see the account. Skipping it is the honest answer; posting it in
     public during a test is not. */
  ...
  if (wanted === 'test' && targets.includes('instagram')) {
    results.instagram = {
      ok: false, skipped: true,
      error: 'Instagram has no private post — there is no API for one and no such '
        + 'thing in the app. To rehearse Instagram, connect a second account you '
        + 'keep private and post to that.',
    };
  }

  if (opts.story) results.story = await alsoStory(env, row, targets, wanted, results);
