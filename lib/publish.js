...
async function publish(env, row, opts = {}) {
   ...
   */
  /*
   * A rehearsal on this road is a Buffer DRAFT, not a private post.
   *
   * Buffer holds the connections and posts under its own platform apps,
   * so TikTok's privacy level is not ours to set here; asking for a
   * private post through Buffer would post publicly. A draft is Buffer's
   * own answer to the same question and a better one: it lands in Buffer
   * with status `draft`, nothing publishes until a person schedules it,
   * and he reads it in the Buffer app on his phone rather than on a
   * TikTok nobody can see.
   */
  if (await postingRoute(env.DB, getSetting) === 'buffer') {
    const results = await toBuffer(env, {
      urls, caption, targets, getSetting, putSetting,
      draft: opts.visibility === 'test',
    ...
    const drafted = list.some((r) => r.ok && r.draft);
    return {
      /* A draft that landed is not a post. Reporting it as posted would
         write posted_at, move the row off the board, and leave him
         believing a rehearsal went out. */
      posted: !drafted && list.some((r) => r.ok),
      drafted,
...
export async function postOne(env, slug, opts = {}) {
  ...
  /*
   * What a rehearsal means depends on the road, and it is the difference
   * between both platforms and one.
   *
   * Through Buffer it is a draft, which every channel takes, so both go.
   * Direct it is a TikTok post at SELF_ONLY, and Instagram has no
   * private post of any kind — not in the API and not in the app — so
   * there the rehearsal is TikTok on its own.
   */
  const igAside = wanted === 'test' && route !== 'buffer' && targets.includes('instagram');
  const row2 = igAside
    ? { ...row, targets: targets.filter((t) => t !== 'instagram').join(',') }
    : row;

  const { posted, drafted, failed, results } = (row2.targets || '').trim()
  ...
  if (igAside) {
    results.instagram = {
      ...
      error: 'not part of a private test on the direct road. Instagram has no '
        + 'private post, so a rehearsal there is TikTok on its own; both go out '
        + 'together when you post publicly.',
    };
  }
  ...
  return { slug: row.slug, visibility: wanted, route, posted, drafted, failed, results };
}
