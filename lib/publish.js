...
import { toAyrshare, postingRoute } from './ayrshare.js';
import { instagramToken, tiktokToken, getSetting, isAudited } from './tokens.js';
...
async function publish(env, row) {
   ...
   *
   * Ayrshare posts to every target in a single call under its own
   * platform apps, which is what makes it useful while TikTok's review
   * of ours is open. It short-circuits everything below: no tokens to
   * resolve, because it holds the connections, and the results come back
   * in the same per-target shape so nothing downstream can tell which
   * road was taken.
   */
  if (await postingRoute(env.DB, getSetting) === 'ayrshare') {
    const results = await toAyrshare(env, { urls, caption, targets });
