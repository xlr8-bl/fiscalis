...
/**
 * The first slide, as a story, after the carousel has gone up.
 *
 * A CAROUSEL cannot BE a story — STORIES and CAROUSEL are different
 * media types and Instagram takes one image or one video for a story.
 * So this is the hook sheet on its own, which is the right slide anyway:
 * it is the cover, it carries no instruction, and a story pointing at a
 * new post is what a story is for.
 *
 * It never fails the carousel. The post is the thing; a story that did
 * not go up is worth reporting and not worth rolling anything back for.
 */
export async function toInstagramStory(env, { url, token, userId }) {
  const id = userId || env.IG_USER_ID;
  if (!id || !token) {
    return { ok: false, skipped: true, error: 'Instagram is not connected yet.' };
  }
  ...
  const container = await post(`${base}/${id}/media`, {
    media_type: 'STORIES', image_url: url, access_token: token,
  });
  if (!container.json?.id) return { ok: false, error: `Story container: ${metaError(container)}` };

  const ready = await waitForContainer(base, container.json.id, token);
  if (!ready.ok) return ready;

  const done = await post(`${base}/${id}/media_publish`, {
    creation_id: container.json.id, access_token: token,
  });
  if (!done.json?.id) return { ok: false, error: `Story publish: ${metaError(done)}` };
  return { ok: true, id: done.json.id };
}
 ...
 *
 * And the audit, which has two halves and it is easy to read only the
 * first: an unaudited client "can only post contents in SELF_ONLY
 * viewership", *and* "all user accounts using the API client to post
 * must be set to private at the time of posting". SELF_ONLY on the post
 * is necessary and not sufficient — a public account is refused however
 * private the post claims to be.
 *
 * Nothing in the API says which side of the audit an app is on, so that
 * is a setting here, defaulting to the safe answer: post privately, and
 * say so in the result rather than reporting a success nobody can see.
 *
 *   https://developers.tiktok.com/doc/content-posting-api-reference-photo-post
 *   https://developers.tiktok.com/doc/content-sharing-guidelines
 */
export async function toTikTok(env, { urls, title, description, token, audited, visibility }) {
  if (!token) {
  ...
  /*
   * The other half of the unaudited rule, and the one that is easy to
   * miss because it is not about the post at all:
   *
   *   "All user accounts using the API client to post must be set to
   *    private at the time of posting."
   *
   * So SELF_ONLY on the post is necessary and not sufficient — the
   * account itself has to be private, or TikTok refuses with
   * unaudited_client_can_only_post_to_private_accounts.
   *
   * creator_info gives it away: only a public account is offered
   * PUBLIC_TO_EVERYONE. Catching it here costs nothing and says the one
   * thing the error code does not, which is what to go and change.
   */
  /* Fires on a test post too. TikTok refuses an unaudited client on the
     ACCOUNT being public, whatever privacy_level is sent, so skipping
     this would trade our message for `unaudited_client_can_only_post_to_
     private_accounts` and nothing about what to change. */
  const options = who.info.privacy_level_options;
  if (!audited && Array.isArray(options) && options.includes('PUBLIC_TO_EVERYONE')) {
    return {
      ok: false,
      error: 'This TikTok account is public, and an unaudited app may only post to a '
        + 'private one. In the TikTok app: Settings and privacy, Privacy, turn on '
        + 'Private account — then post again. It can go back to public once the '
        + 'app passes its audit.',
    };
  }

  /* `visibility: 'test'` asks for the quietest level there is, whatever
     the audit state. It is what makes a real posting test possible: the
     post genuinely goes to TikTok, and only he can see it. */
  const wanted = visibility === 'test' ? 'SELF_ONLY'
    : audited ? (env.TIKTOK_PRIVACY || 'PUBLIC_TO_EVERYONE') : 'SELF_ONLY';
  const privacy = choosePrivacy(options, wanted);
...
function tiktokError(error) {
  ...
  const explain = {
    ...
    unaudited_client_can_only_post_to_private_accounts:
      'The account has to be set to private while the app is unaudited — that is a '
      + 'setting on the TikTok account, not on the post. Settings and privacy, '
