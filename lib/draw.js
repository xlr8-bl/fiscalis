 ...
 */
export async function drawCarousel(env, slug, { only = null, fetcher, key, model } = {}) {
  ...
  // what it cost, said out loud. There is no free tier on any of these
  // models, so a run that quietly draws twelve slides is money spent
  // without anyone deciding to spend it.
  const drewWith = drawn.length ? (model || null) : null;
  const spent = drewWith ? costOf(drewWith, drawn.length) : null;
