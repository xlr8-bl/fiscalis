...
import { paintGround, NO_AI, MODEL as PAINT_MODEL } from './paint.js';
...
export async function drawCarousel(
  ...
  const owed = carousel.slides.filter(
  ...
  /*
   * A slide carrying a `design` is not for this road.
   *
   * Teaching panels and hook sheets are drawn on a canvas in the studio:
   * measured geometry, real type, his photograph cut out. They need no
   * image model, and this used to hand them to one anyway because it
   * filtered on state and never looked at `design`.
   *
   * With no model bound that surfaced as "Workers AI is not bound" on a
   * carousel that never needed it, which is the confusing outcome. The
   * expensive one is the other way round: with a model bound, every
   * panel would have come back an AI picture with no words on it, and
   * been marked drawn.
   */
  const designed = owed.filter((s) => s.design);
  const wanted = owed.filter((s) => !s.design);

  if (!wanted.length) {
    return {
      slug,
      drawn: 0,
      not_for_this_tool: designed.length,
      note: 'Nothing here is for the image model. These slides are teaching '
        + 'panels or hook sheets: they are drawn on a canvas in the studio, with '
        + 'measured type rather than a generated picture, and a person does it in '
        + 'a tap. Hand the carousel over instead. `draw` is for the older path '
        + 'where a slide IS a generated picture.',
    };
  }

  /*
   * Now, and only now, is a model actually wanted: something here has no
   * design and an image model is the only thing that can draw it. The
   * callers used to ask this FIRST, which is how a carousel of teaching
   * panels was answered with "Workers AI is not bound" — the gate stood
   * in front of the code that knew whether a model was needed.
   *
   * And when there is no model, WHICH sentence to say depends on the
   * carousel. One where nothing at all carries a design predates the
   * templates: naming the binding is true and useless, because the real
   * answer is that it was made the old way and should be made again.
   * A carousel with a model available still draws either way.
   */
  const noModel = free ? (!env.AI && !runner) : !key;
  if (noModel) {
    const allOld = !carousel.slides.some((s) => s.design);
    if (allOld) {
      return {
        slug,
        drawn: 0,
        made_the_old_way: true,
        error: 'This carousel was made on the older path, before the templates: '
          + 'its slides carry a prompt for an image model and no arrangement, so '
          + 'there is nothing for the canvas to draw. Ask Spark to make it again '
          + '— it can only use teach_carousel now, and those slides draw here for '
          + 'free with measured type. Then kill this one. (Binding an image model '
          + 'would draw it too, but as a generated picture rather than a panel.)',
      };
    }
    return {
      slug,
      drawn: 0,
      error: free ? NO_AI
        : 'No Gemini API key is set, and these slides need an image model. Add it '
          + 'under Accounts, or switch that screen to Cloudflare.',
    };
  }
  ...
  return {
    ...
    // mixed carousels exist; say what was left alone rather than
    // reporting a partial draw as a whole one
    ...(designed.length ? {
      left_for_the_studio: designed.length,
      why: 'Those slides carry a design, so they are drawn on a canvas in the '
        + 'studio rather than by an image model.',
    } : {}),
