 ...
 * Ask Unsplash.
 *
 * `orientation=landscape` because every cover on this site is 1200x630,
 * and a portrait photograph cropped to that loses whatever it was of.
 */
async function fromUnsplash(query, key, count) {
  const url =
  ...
  return (data.results ?? []).map((p) => ({
    ...
    url: `${p.urls.raw}&w=${COVER.width}&h=${COVER.height}&fit=crop&crop=entropy&q=80&fm=jpg`,
    page: p.links?.html ?? '',
...
/**
 * Score a candidate as a cover.
 *
 * Aspect does most of the work, because the failure this fixes is a
 * portrait scan winning a search for a landscape picture. 1.9 is the
 * cover's own ratio; anything at or past it crops without loss.
 */
function score(p) {
  let s = 0;
  if (p._unsplash) s += 60;          // an actual photographer's photograph
  if (p._free) s += 15;              // CC0 or public domain: no credit line owed
  const ratio = p.height ? p.width / p.height : 1.9;
  if (ratio >= 1.6) s += 40;
  else if (ratio >= 1.3) s += 20;
  else if (ratio >= 1.1) s += 5;
  else s -= 40;                      // portrait: cannot be a 1200x630 cover
...
async function fromCommons(query, count) {
  ...
  for (const search of passes(query)) {
    ...
    if (pool.filter((p) => score(p) >= 60).length >= count) break;
  }

  return pool
    .filter((p) => !p.height || p.width / p.height >= 1.1)
 ...
 */
export async function findPhotos(env, query, count = 6) {
  ...
  if (env.UNSPLASH_ACCESS_KEY) {
    ...
    try {
      photos = await fromUnsplash(q, env.UNSPLASH_ACCESS_KEY, count);
    } catch (err) {
  ...
  if (!photos.length) {
    ...
    try {
      photos = await fromCommons(q, count);
    } catch (err) {
