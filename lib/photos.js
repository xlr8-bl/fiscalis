...
async function fromUnsplash(query, key, count, shape = 'wide') {
  ...
  return (data.results ?? []).map((p) => ({
    ...
    // It is built on urls.raw, which is the URL the API returned: the
    // guidelines require the photograph to come from their host, and
    // that is also what keeps the photographer's view count real.
    url: `${p.urls.raw}&w=${size.width}&h=${size.height}&fit=crop&crop=faces,entropy&q=80&fm=jpg`,
    page: withUtm(p.links?.html ?? ''),
    /* The endpoint that counts the download. Unsplash asks for a GET to
       it whenever a photograph is actually taken and used, and it is the
       only way a photographer sees that their picture was used. It is
       not the file: hitting it downloads nothing. Carried here so that
       keepPhoto, which is where the taking happens, can call it. */
    countAt: p.links?.download_location ?? '',
    ...
    byLink: withUtm(p.user?.links?.html ?? ''),
...
/* Unsplash asks that links back carry these, so a photographer can see
   where their traffic came from. It costs nothing and it is the half of
   attribution that a credit line on its own does not give them. */
const withUtm = (href) => (href
  ? `${href}${href.includes('?') ? '&' : '?'}utm_source=web3ashley&utm_medium=referral`
  : '');
...
export async function keepPhoto(env, photo, { slug = 'cover' } = {}) {
  ...
  /*
   * Tell Unsplash the photograph was taken.
   *
   * This is the point where it happens: a search is browsing and costs
   * the photographer nothing, and THIS is somebody choosing their
   * picture and keeping it. Their download count is how they see that,
   * and it is the one thing in the guidelines that cannot be inferred
   * from traffic.
   *
   * It never fails the save. A photograph in the library and an
   * uncounted download is a small debt; a photograph lost because a
   * counter was down is a broken tool.
   */
  if (photo.source === 'unsplash' && photo.countAt && env.UNSPLASH_ACCESS_KEY) {
    try {
      await fetch(photo.countAt, {
        headers: {
          authorization: `Client-ID ${env.UNSPLASH_ACCESS_KEY}`,
          'accept-version': 'v1',
        },
      });
    } catch { /* counted or not, the picture is kept */ }
  }
  ...
  return {
    ...
    credit: photo.by
      ...
      : photo.licence,
    /* Unsplash's guidelines want the photographer named, their profile
       linked and Unsplash itself named. The credit line above carries
       the name; these carry the two links, so whatever renders it can
       make both without going back to the API. */
    creditLink: photo.byLink || '',
    homeLink: photo.source === 'unsplash'
      ? 'https://unsplash.com/?utm_source=web3ashley&utm_medium=referral' : '',
