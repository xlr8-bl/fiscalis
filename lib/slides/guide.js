...
function roomIn(name) {
  ...
  const fits = (n) => {
    ...
    const says = blocks.filter((b) => b === 'say').length;
    const each = Math.max(1, Math.ceil(n / Math.max(1, says)));
    const text = Array.from({ length: each }, () => line).join(' ');
    slide.say = text;
    if (says > 1) slide.say2 = text;
    return validateSlides({ slides: [slide, slide] }).ok;
  };
