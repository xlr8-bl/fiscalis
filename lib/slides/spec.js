...
export const LIMITS = {
  ...
  swapLine: { max: 46 },
  tag:      { max: 14, note: 'it is reversed out of a slab in the foot band' },
  foot:     { max: 58, note: 'one line, and the band is one line tall' },
  stack:     { min: 2, max: 5 },
...
export const SLIDE_FIELDS = [
  ...
  'unit',
  /* The foot band's own two. It draws without them; these fill it. */
  'tag', 'foot',
...
function sayWhat(problems, headline, at, what) {
  ...
  if (!CONCRETE.test(headline)) {
    problems.push(
      ...
      + 'See opening.before_and_after in the work order.'
    );
  }
...
function checkHook(problems, slide, at, isFirst) {
  ...
  /*
   * A bare number in ANY slot, not just `index`.
   *
   * Several sheets carry an `index` from their reference, where it was
   * an issue number, and that slot was refused. So it arrived as
   * "№ 044" in a different slot and drew on the front of a real
   * carousel: the numero sign is not "no.", and 044 is three digits, so
   * the written-down numbering ban missed it twice over.
   *
   * A figure on its own in a display slot reads as a count whatever it
   * was meant to be, and this is a rule that has been asked for twice.
   */
  for (const name of named) {
    const v = String(slide[name] ?? '').trim();
    if (!v) continue;
    if (/^(?:№|#|no\.?)?\s*\d{1,4}$/i.test(v)) {
      problems.push(
        `${at}: ${name} is "${v}", which is a number on its own. A carousel never `
        + 'numbers itself, and a figure in a display slot reads as a count '
        + 'whatever it was meant to be. Write words there or leave it out.'
      );
    }
  }
...
export function validateSlides(spec, { opener = true } = {}) {
    ...
    for (let i = 1; i < slides.length - 1; i++) {
      ...
      if (a && a === b) {
        problems.push(
          ...
          + 'the two is carrying evidence should be showing it instead. '
          + 'the_other_templates in the work order indexes the rest by the job '
          + 'each does; `template` returns one in full.'
        );
      }
    }
  }
  ...
  const plan = slides.map((slide, i) => {
    ...
    /* The foot band is on every teaching slide now, so the stack ends
       above it rather than at the frame's foot. */
    const bottom = 1 - M.band.h - M.foot;
    const room = (pinned ? bottom - pinned - M.gap.between : bottom)
      - M.top - MARGIN;
...
function checkBlock(problems, at, block, key, value) {
  ...
  if (block === 'icons') {
    ...
    if (bad.length) {
      /* Named here rather than pointed at. A refusal that says where
         to go costs a second call and the guide it pointed at has moved
         once already. */
      problems.push(`${at}: no icon called ${bad.join(', ')}. The pack is: `
        + `${ICON_NAMES.join(', ')}.`);
      return;
    }
