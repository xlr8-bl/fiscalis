...
/** An odd number reads as measured; any number is at least concrete. */
const COUNTED = /\d|\b(one|two|three|four|five|six|seven|eight|nine|ten|eleven|twelve)\b/i;

/**
 * @param opts.opener  judge the first slide's headline. Off for the
 *   internal probes that measure geometry: they fill a slide with
 *   nonsense prose to find where it stops fitting, and refusing that
 *   nonsense for being vague makes the measurement come back zero.
 */
export function validateSlides(spec, { opener = true } = {}) {
  ...
  if (headline && !CONCRETE.test(headline) && !COUNTED.test(headline)) {
    problems.push(
      `Slide 1: "${headline}" names no thing and counts nothing, so it reads as `
      + 'a label on a topic rather than a hook. Name what it is about — a form, '
      + 'a booking page, the hours on Google — or count something, and hold back '
      + 'what it costs instead. See opening.before_and_after in design_brief.'
    );
  }
