...
function costOf(block, value) {
  ...
  switch (block) {
    ...
    case 'icons':  return M.icons.h;
    case 'action': {
      const n = wraps(value, PER_LINE.action);
      return M.action.label + 0.006 + M.action.body * CAP
        + (n - 1) * M.action.body * M.action.lead + M.action.padY * 2;
    }
    default: return 0;
  }
}
