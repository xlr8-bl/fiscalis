...
function checkHook(problems, slide, at, isFirst) {
  ...
  for (const name of named) {
    ...
    const n = String(Array.isArray(text) ? text.join(' ') : text).length;
    if (max && n > max) {
      problems.push(
        `${at}: ${name} is ${n} characters and ${id} holds ${max}. Measured off the `
...
export function validateSlides(spec, { opener = true } = {}) {
  ...
  if (headline) {
    ...
    if (!CONCRETE.test(headline)) {
      problems.push(
        `Slide 1: "${headline}" names nothing a person could point at on their `
        + 'own screen, so it reads as a label on a topic rather than a hook. Name '
        + 'the thing — a form, a booking page, the hours on Google, a checkout — '
        + 'and hold back what it costs instead. Counting is not enough on its own. '
    ...
    if (abstract) {
      problems.push(
        `Slide 1: "${abstract[0]}" describes the work instead of naming it, and a `
