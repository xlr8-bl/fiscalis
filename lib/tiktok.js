...
/**
 * The credentials, trimmed.
 *
 * A value pasted into a dashboard field can arrive with a trailing space
 * or a newline on it, and nothing about that is visible anywhere: the
 * key looks right in the box, gets percent-encoded into the authorize
 * URL, and TikTok answers "correct the following: client_key" with no
 * hint that the problem is one invisible character. So it is stripped
 * here rather than trusted.
 */
export const clientKey = (env) => String(env.TIKTOK_CLIENT_KEY || '').trim();
export const clientSecret = (env) => String(env.TIKTOK_CLIENT_SECRET || '').trim();

/**
 * Which client to act as.
 *
 * A sandbox and the live app are separate clients with separate keys,
 * and the approval path runs through both: you build and record the demo
 * against a sandbox, because a sandbox needs no review, then submit and
 * eventually move to the live one. That is two credential swaps.
 *
 * Doing those in the Pages environment means two dashboard edits and two
 * deployments, from a phone, each one a wait — so the pair can be set in
 * the studio instead and is stored like the tokens are. The environment
 * stays the fallback, which is what a fresh deployment starts from.
 */
export async function credentials(db, env, { getSetting } = {}) {
  if (db && getSetting) {
    const key = (await getSetting(db, 'tiktok.client_key')) || '';
    const secret = (await getSetting(db, 'tiktok.client_secret')) || '';
    if (key.trim() && secret.trim()) {
      return { key: key.trim(), secret: secret.trim(), source: 'studio' };
    }
  }
  return { key: clientKey(env), secret: clientSecret(env), source: 'environment' };
}
...
export function authorizeUrl(env, { origin, state, key }) {
  const url = new URL(AUTHORIZE);
  url.searchParams.set('client_key', key ?? clientKey(env));
...
export function exchange(env, { origin, code, key, secret }) {
  return tokenCall({
    client_key: key ?? clientKey(env),
    client_secret: secret ?? clientSecret(env),
    code,
...
/**
 * Ask TikTok whether these credentials are real.
 *
 * `client_credentials` validates the key and secret on their own, with
 * no user, no redirect and no Login Kit involved. That makes it the one
 * question worth asking when the authorization page answers "correct the
 * following: client_key" — because it splits the two halves of that
 * error apart:
 *
 *   a token comes back    the key and secret are a valid pair, so the
 *                         authorize failure is about how the app is
 *                         configured for the web, not about the value
 *   an error comes back   the value itself is wrong, and TikTok says how
 *
 * Guessing between those two costs a deploy each time. This costs one
 * request.
 *
 *   https://developers.tiktok.com/doc/client-access-token-management
 */
export async function testCredentials(env, { fetcher = fetch, key, secret } = {}) {
  key = (key ?? clientKey(env)).trim();
  secret = (secret ?? clientSecret(env)).trim();
  if (!key || !secret) return { ok: false, error: 'The key or the secret is not set.' };

  try {
    const res = await fetcher(TOKEN, {
      method: 'POST',
      headers: { 'content-type': 'application/x-www-form-urlencoded' },
      body: form({ client_key: key, client_secret: secret, grant_type: 'client_credentials' }),
    });
    const json = await res.json().catch(() => ({}));
    if (json.access_token) return { ok: true, expiresIn: Number(json.expires_in) || 0 };
    return {
      ok: false,
