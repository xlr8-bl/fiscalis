 ...
 *
 * Cloudflare's REST screenshot endpoint, not the Workers binding: Pages
 * Functions cannot take a `browser` binding, and the REST road needs
 * only an API token, which is the same shape the paid image path already
 * uses. It works on the free plan — 10 minutes of browser time a day,
 * one Quick Action every 10 seconds — which is a few dozen captures and
 * far more than a carousel needs.
 *
 * ON THE NAME. The product is Browser Run since April 2026 and was
 * Browser Rendering before it. The rename did not reach the wiring: the
 * endpoint below still has `browser-rendering` in its path and the API
 * token permission is still written `Browser Rendering - Edit` in the
 * docs, while the dashboard may offer it as Browser Run. Same thing,
 * either name, and the URL here is not a typo.
 *
  ...
  if (!account || !token) {
    return {
      error: 'Capturing a page needs CF_ACCOUNT_ID and CF_BROWSER_TOKEN set in '
        ...
        + 'needs the Browser Run permission at Edit, which some of Cloudflare\'s '
        + 'own pages still call Browser Rendering. Nothing else on this '
