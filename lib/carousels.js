...
import { VOICE, RESEARCH, SELF_CHECK, CLOSES, ANCHORS, voiceRules }
  from '../assets/js/brand.js';
...
/**
 * What a person may do that the graph above does not cover.
 *
 * `review` exists so the agent has somewhere to hand work over *to*. A
 * carousel a person made themselves has nobody else to hand it to, and
 * making them push it through review to approve their own pictures is
 * ceremony. So the studio may approve straight out of the making states.
 *
 * This widens nothing for the agent: mayMove checks the kind first, and
 * `approved` is not one of AGENT_STATES.
 */
const STUDIO_EXTRA = {
  planned: ['approved'],
  generating: ['approved'],
...
export function mayMove(kind, from, to) {
  if (kind === 'studio' && STUDIO_EXTRA[from]?.includes(to)) return true;
...
export async function getCarousel(db, slug) {
  ...
  // the format and the weight come from the media row rather than being
  // copied onto the slide, so what is checked against Instagram's limits
  // is what is actually in the bucket
  const { results } = await db
    .prepare(
      `SELECT s.id, s.position, s.kind, s.copy, s.prompt, s.media_key,
              s.ground_key, s.design, s.width, s.height, s.state, s.note, s.qc,
              s.attempts, s.updated_at, m.content_type, m.bytes
       FROM slides s LEFT JOIN media m ON m.key = s.media_key
       WHERE s.carousel_id = ?1 ORDER BY s.position`
    )
    .bind(row.id)
    .all();

  // the pictures the generator draws with: `likeness` are keyed cut-outs
  // of the operator, `aesthetic` are scenes a panel can be built on. Sent
  // with the carousel so the studio does not need a second round trip
  // before it can draw anything.
  const { results: refs } = await db
    .prepare(`SELECT media_key, role FROM brand_refs
              WHERE active = 1 ORDER BY role, position`)
    .all()
    .catch(() => ({ results: [] }));
  const scenes = [];
  const cutouts = [];
  for (const r of refs ?? []) {
    (r.role === 'likeness' ? cutouts : scenes).push(`/media/${r.media_key}`);
  }
  ...
  return {
    ...
    slides: (results ?? []).map((s) => ({
      ...
      // the drawn picture before the copy went over it. Present only on
      // the free path, and only until the studio has set the type.
      ground: s.ground_key ? `/media/${s.ground_key}` : '',
      needs_type: Boolean(s.ground_key && !s.media_key),
      // the generator's spec for this panel. A slide arrives with either
      // a ground to typeset over or a design to draw from, never both,
      // so the two flags cannot both be true.
      design: parseJson(s.design, null),
      needs_design: Boolean(s.design && !s.media_key),
    })),
  };
}
...
export async function setSlides(db, carouselId, slides) {
  ...
  const stmt = db.prepare(
    `INSERT INTO slides (carousel_id, position, kind, copy, prompt, design, state)
...
export async function brief(db, { site = '' } = {}) {
  ...
  return {
    // the rules it writes against, each saying whether the server will
    // catch it or whether passing means nothing
    voice: voiceRules(),
    research: RESEARCH,
    self_check: SELF_CHECK,
    /* Shapes, not a line. A fixed sign-off asks for nothing and stops
       being read by the fourth post; the close is written out of each
       carousel's own subject. */
    closes: CLOSES,
    anchors: ANCHORS,
    pillars: pillars ?? [],
    references,
    ...
    slots: SLOTS,
    /*
     * How long a carousel runs is the topic's call, not a house number.
     * A teardown with four findings is four slides plus a hook; a single
     * statistic is two. Padding to a fixed length is how a set of posts
     * starts to look like a template, which is the thing the whole
     * pipeline exists to avoid.
     *
     * The floor and ceiling are Instagram's, not a preference: below two
     * it is not a carousel, and ten is the platform's limit.
     */
    slides: {
      min: MIN_SLIDES,
      max: MAX_SLIDES,
      note: 'Choose the length from the topic. Do not pad to a fixed count.',
      /*
       * One file has to satisfy both platforms, and the tighter of the
       * two constraints is TikTok's: photos are capped at 1080p and it
       * does not downscale, where Instagram takes a larger file and
       * downscales it to 1440 anyway. So 1080 wide is the size at which
       * a single master posts everywhere.
       *
       * 4:5 because that is the tallest ratio an Instagram feed carousel
       * accepts — 9:16 is a Reel, and is refused here.
       */
      size: {
        width: 1080,
        height: 1350,
        ratio: '4:5',
        format: 'image/jpeg',
        why: 'TikTok caps photos at 1080p and does not resize; Instagram takes JPEG '
          + 'only and 4:5 is the tallest a feed carousel accepts. One file, both places.',
      },
    },
  };
}
