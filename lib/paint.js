 ...
 */
/*
 * Said once, because it was said three times and one of them was wrong.
 *
 * `lib/paint.js` used to say "add an [ai] binding to wrangler.toml",
 * which is the one thing that must not be done: Workers AI has no local
 * implementation, so declaring it there makes `wrangler pages dev` open
 * a remote proxy session and every local run and check suite dies at
 * startup. The binding is added in the dashboard on purpose, and the
 * refusal has to name where, or it is a dead end on a phone.
 */
export const NO_AI =
  'Workers AI is not bound on this deployment. In the Cloudflare dashboard: '
  + 'Workers and Pages, web3ashley, Settings, Functions, Bindings, Add, Workers '
  + 'AI, name it AI. Then make a NEW deployment, because a binding reaches a '
  + 'deployment the way a secret does. Or switch to Google under Accounts, '
  + 'which needs a key and no binding.';

export async function paintGround(env, { slide, model: pick, steps, runner } = {}) {
  const ai = runner || env.AI;
  if (!ai) return { error: NO_AI };
