...
export const TOOLS = [
  ...
  },
  {
    ...
    description:
      'START HERE when asked for a carousel. One call and the server has already '
      + 'made the five decisions that are not yours to take differently every time: '
      + 'which pillar is due, which hook sheet has not been used lately, the six '
      + 'slide shape, which outro is next in the rotation, and the rail. It also '
      + 'returns the topics not to repeat, the exact fields every chosen sheet and '
      + 'template takes with their measured character limits, and the rules that '
      + 'will refuse you. What is left to you is the research and the words. '
      + 'Optionally name a pillar or a topic to steer it.',
    inputSchema: {
      type: 'object',
      properties: {
      ...
      properties: {
        ...
        slides: {
          ...
          items: {
            type: 'object',
            properties: {
              template: str('Which arrangement. Pick by the JOB the slide has to '
                + 'do, which design_brief lists as `for` against each one: saying '
                + 'what the subject is, showing a real page, teaching a word, '
                + 'drawing figures to scale, instead-of/do-this, ending the set.'),
              ...
              action: str('The one thing to actually do. ONLY on the templates '
                + 'whose job is an instruction — swap, close, signoff, recap, '
                + 'calling, bookend. On a teaching panel it is refused by name: '
                + 'DO THIS: on every slide gives a reader five orders and none '
                + 'of them lands. A set carries one ask, at the end.'),
              ...
              shot: {
                ...
                properties: {
                  ...
                  url: str('The address that was captured. Required: the address '
                    + 'bar IS the citation on this slide.'),
              ...
              quote: {
                ...
                properties: { text: str('What they actually wrote or said.'),
                              who: str('Who, and where. Required: an unattributed '
                                + 'quotation is the shape of evidence with nothing in it.') },
                description: 'For the quote template. Never invented, never tidied up.',
              },
              apps: {
                type: 'array', minItems: 2, maxItems: 5,
                items: { type: 'string' },
                description: 'App marks, for the tools template. A mark may only NAME '
                  + 'a product this slide discusses. Names are in the brief.',
              },
              swap: {
