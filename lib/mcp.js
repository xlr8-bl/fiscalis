 ...
 * One road, not two.
 *
 * `plan_carousel` and `draw` are the path from before the templates
 * existed, where a slide WAS a generated picture and an image model made
 * it. They are out of the default scope now, and it is not only that
 * they are unused: an AI picture behind the type destroys the thing the
 * templates are for. Every ground in both engines is a flat colour with
 * a measured contrast ratio — paper and ink at 15.89:1, red refused for
 * body copy at 4.62 — and a picture has no ratio to check.
 *
 * Spark kept taking the fork. It is told in INSTRUCTIONS which path is
 * the old one, and it called `draw` on a teaching carousel anyway, which
 * is this project's one recurring lesson: instructions do not work,
 * refusals do. Out of scope is the refusal.
 *
 * Neither tool is deleted. Both answer under the `everything` scope, the
 * same way post_due does, so it is one setting to get back.
 */
export const CAROUSEL_TOOLS = [
  'next_carousel', 'brief', 'design_brief', 'queue', 'list_carousels',
  'teach_carousel', 'design_carousel', 'design_status',
  'deliver_slide', 'add_reference', 'check_posting', 'hand_over',
