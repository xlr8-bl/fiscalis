...
import { HOOK_SLOTS } from '../assets/js/hooks/guide.js';
      ...
      properties: {
        ...
        slides: {
          ...
          items: {
            ...
            properties: {
              /*
               * A slide is EITHER a template or a hook sheet, and the
               * schema never said so: `hook` was not a field here and
               * `template` was required, while the validator refused any
               * first slide that was not a sheet. There was no way to
               * satisfy both, so the only route was guesswork — one run
               * spent its whole budget trying `template: "h004"`, then
               * `sheet`, then reading errors for a field that did not
               * exist, and filed a carousel with no slides.
               */
              template: str('Which arrangement, picked by the JOB the slide has '
                + 'to do. The work order lists them with a `for` each. Leave it '
                + 'out ONLY on slide one when you are using `hook` instead.'),
              hook: str('A hook SHEET id, like "h051". Slide one only, and slide '
                + 'one is always one of these rather than a template. The work '
                + 'order picks which sheet in `slide_1` and lists its slots with '
                + 'a character ceiling each: write those slot names as fields on '
                + 'this same slide. Do not put a sheet id in `template`.'),
              ...
              /* The hook sheets' slots, named so the schema accepts them
                 and bare so they cost nothing: which ones a sheet takes,
                 what each wants and its character ceiling all come back
                 in the work order's `slide_1`. A description on each of
                 the fifty would be the same sentence fifty times, which
                 is five kilobytes on every call. */
              ...Object.fromEntries(HOOK_SLOTS.map((n) => [n, { type: 'string' }])),
            },
            /* Nothing is required, because a slide is either a template
               or a hook sheet and requiring `template` made the sheet
               unexpressible. The validator names whichever is missing. */
          },
        },
      },
      required: ['title', 'slides'],
...
export const INSTRUCTIONS =
  ...
  + 'task. Send the WORDS, name the template, and let the site draw.\n\n'
  ...
  + 'There is ONE road to a carousel and it is the one above. Every slide is '
  + 'drawn on a canvas from the template you named: measured type on a flat '
  + 'ground whose contrast is known. No image model is involved and none should '
  + 'be. deliver_slide is only for an image a person handed you. queue lists '
  + 'slides asked for again; redo only those.\n\n'
 ...
 * One road, not two.
 *
 * THREE roads existed and Spark took a different one each time.
 *
 *   plan_carousel  a slide IS a generated picture. Needs an image model
 *                  and produced carousels nothing could draw.
 *   design_carousel  setup/payoff/body on a device mockup. Draws fine,
 *                  on the canvas, and is not the templates at all: it
 *                  came out looking like a different account.
 *   teach_carousel   the templates. The one that is wanted.
 *
 * The first two are out of the default scope AND refused behind a
 * setting, because out of scope alone did not hold: on a deployment
 * whose `agent.scope` is `everything` they are still listed, and Spark
 * called them. INSTRUCTIONS said there was one road and, four paragraphs
 * earlier, offered design_carousel by name. That contradiction is gone.
 *
 * Nothing is deleted. Everything answers under `everything` with the
 * setting on, the same way post_due does.
 */
export const CAROUSEL_TOOLS = [
  'next_carousel', 'brief', 'queue', 'list_carousels',
  'teach_carousel', 'deliver_slide', 'add_reference', 'check_posting',
  'hand_over', 'progress', 'capture_page', 'template',
];
