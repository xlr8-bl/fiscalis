...
import { TEMPLATE_GUIDE, TEMPLATE_INDEX, COMMON_FIELDS, HOW_TO_OPEN, HOW_TO_CLOSE,
         PICK_A_SLIDE, THE_TWO_FACES, HOW_TO_WRITE, REFUSALS, perLine }
  from './slides/guide.js';
import { WHICH_PACK } from '../assets/js/icons.js';
 ...
 *
 * `shapes` is which templates come back in full. All 23 cost about four
 * thousand tokens before a word was written and a carousel uses six; the
 * rest are one line each and fetchable by name.
 */
export const ANGLES = [
  { name: 'take one real thing apart',
    ...
    shapes: ['shot', 'annotated', 'reasons', 'proof', 'swap'] },
  { name: 'two ways, side by side',
    ...
    shapes: ['compare', 'swap', 'reasons', 'proof', 'tools'] },
  { name: 'something that changed',
    ...
    shapes: ['shot', 'reasons', 'tools', 'figure', 'steps'] },
  { name: 'the thing everyone repeats that is wrong',
    ...
    shapes: ['myth', 'proof', 'swap', 'quote', 'reasons'] },
  { name: 'a number and what is behind it',
    ...
    shapes: ['figure', 'numbers', 'chart', 'reasons', 'proof'] },
  { name: 'the checklist to run today',
    ...
    shapes: ['steps', 'checklist', 'shot', 'roundup', 'tools'] },
];
...
export async function workOrder(db, { pillar = null, topic = null } = {}) {
  ...
  /* The openers, the shapes this angle wants, and the sign-off. The rest
     are one line each below, fetchable by name. */
  const wanted = [...new Set(['open', 'define', 'quote', 'shot',
                              ...angle.shapes, OUTRO])];
  const inFull = TEMPLATE_GUIDE(wanted);
  const rest = Object.fromEntries(Object.entries(TEMPLATE_INDEX())
  ...
  return {
    ...
    /*
     * Slide one as a FORM, not a description to work out a shape from.
     *
     * `slide_1` used to be the catalogue entry: family, what it does,
     * what to avoid, a slots array. All true, and none of it said what
     * to SEND. One run spent its whole budget discovering that a sheet
     * goes in `hook` rather than `template` and filed a carousel with no
     * slides. The object below is the slide, with the blanks named.
     */
    slide_1: {
      send_exactly_this_shape: {
        hook,
        ...Object.fromEntries((sheet?.slots ?? []).map((s) => [s.name, '...'])),
      },
      fill_in: Object.fromEntries((sheet?.slots ?? []).map((s) => [s.name, {
        at_most: s.characters,
        write: s.wants ?? 'Short. See the sheet.',
        ...(s.wraps ? { wraps: true } : { one_line: true }),
      }])),
      no_template_field: 'Slide one is a hook SHEET. It takes `hook`, not '
        + '`template`, and the sheet id does not go in `template`.',
      what_the_sheet_is: sheet?.does ?? '',
      avoid: sheet?.avoid ?? '',
      ...(sheet?.his_photograph ? { his_photograph: sheet.his_photograph } : {}),
    },
    /* Without the by-what-it-does map: every template below carries the
       same phrase as `for`, and the ones not sent in full are that map
       already. */
    how_to_choose: { ...PICK_A_SLIDE, by_what_the_slide_has_to_do: undefined },
    what_every_slide_takes: COMMON_FIELDS,
    templates_you_may_use: inFull,
    /* Named and one line each rather than spelled out. Every one of them
       is a real template and `template` returns the same detail the ones
       above carry; what is saved is the four thousand tokens of
       arrangements this carousel was never going to use. */
    the_other_templates: rest,
    to_use_one_of_those: 'Call `template` with its name. It costs one call '
      + 'and it is the right move whenever the subject wants a shape that is '
      + 'not in the list above. Do not guess its fields.',
    ...
    /* The closing rule in full. It used to be `the_rule` alone, which
       said "there is no fixed sign-off" while the validator refused
       anything but `signoff` — a refusal on every carousel written to
       it. */
    ...
    /* These lived in design_brief, which is the OTHER engine's brief and
       no longer carries the teaching guide. Without them here the type
       rule and the two icon packs are unreachable from the only road a
       teaching carousel is written on. */
    /* Without `the_shape_of_a_set`: every line of it is said elsewhere in
       this same payload, and its first line ("five or six slides")
       contradicts the storyboard above, which is however many the
       subject needs. */
    how_it_is_set: {
      the_two_faces: THE_TWO_FACES,
      characters_a_line: perLine(),
      writing: { ...HOW_TO_WRITE, the_shape_of_a_set: undefined },
    },
    /* The meanings, not only the names: an icon is picked for the
       sentence it sits under, and a list of bare names invites picking
       by shape. They used to live in design_brief, which is the other
       engine's. */
    icons_and_marks: {
      which_pack: WHICH_PACK,
      the_icons: slideCatalogue().icons,
      app_marks: USE_A_MARK,
    },
    what_else_refuses: REFUSALS,
