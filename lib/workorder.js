...
export async function workOrder(db, { pillar = null, topic = null } = {}) {
  ...
  return {
    ...
    the_shape: [
      ...
      /* Named by the JOB, not by the template, and no template named
         twice: picking by name meant taking whichever came first, which
         is how three sets in a row were six panels of reasons. */
      { slide: 2, use: 'template', the_job: 'say what the subject is',
        pick_from: ['open', 'define', 'quote'] },
      { slide: 3, use: 'template', the_job: 'why it matters, or what is actually there',
        pick_from: ['reasons', 'shot', 'compare'] },
      { slide: 4, use: 'template', the_job: 'the evidence',
        pick_from: ['chart', 'figure', 'proof', 'shot', 'steps'] },
      { slide: 5, use: 'template', the_job: 'what to do about it',
        pick_from: ['swap', 'annotated', 'tools', 'portrait'] },
      { slide: 6, use: 'template', template: OUTRO, set_exactly: OUTRO_LOOK,
        why: 'The outro, and the only slide whose look is fixed: him seated with '
           + 'the phone, on yellow. Same sign-off every time, so a reader knows a '
           + 'set has ended. You write the words and nothing else. One ask, out of '
           + 'this carousel\'s own subject.' },
    ],
    ...
    templates_you_may_use: only([
      'open', 'reasons', 'steps', 'proof', 'compare', 'portrait',
      'shot', 'annotated', 'chart', 'figure', 'define', 'quote', 'tools',
      'swap', OUTRO,
    ]),
