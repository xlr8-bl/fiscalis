...
import { clientKey, clientSecret } from './tiktok.js';
...
export async function tiktokToken(db, env) {
  ...
  if (!env.TIKTOK_CLIENT_KEY || !env.TIKTOK_CLIENT_SECRET) {
    return {
      token: access || null,
      error: 'TIKTOK_CLIENT_KEY and TIKTOK_CLIENT_SECRET are needed to renew the token.',
    };
  }
...
export async function accountState(db, env) {
  ...
  return {
    ...
    tiktok: {
      ...
      can_renew: Boolean(env.TIKTOK_CLIENT_KEY && env.TIKTOK_CLIENT_SECRET),
      // the app is set up well enough to run the connect flow at all
      can_connect: Boolean(env.TIKTOK_CLIENT_KEY && env.TIKTOK_CLIENT_SECRET),
