...
import { postingRoute } from './buffer.js';
import { credentials as igApp } from './instagram.js';
import { apiKey as geminiKey, imageModel, drawProvider, MODELS } from './imagen.js';
import { MODEL as PAINT_MODEL, NEURONS, FREE_DAILY_NEURONS } from './paint.js';
...
export async function accountState(db, env) {
  ...
  const app = await credentials(db, env, { getSetting });
  const ig = await igApp(db, env, { getSetting });
  const gem = await geminiKey(db, env, { getSetting });
  const draws = await imageModel(db, env, { getSetting });
  const how = await drawProvider(db, env, { getSetting });
  const days = (s) => Math.max(0, Math.round(s / 86400));

  return {
    // the generation layer: without a key nothing can be drawn at all,
    // which is a different kind of stuck from "not connected"
    drawing: {
      provider: how.provider,
      provider_source: how.source,
      // The free path needs no key at all, so "ready" has to mean two
      // different things. Reporting Gemini's key state while Cloudflare
      // is drawing would say "not ready" about a path that is working.
      ready: how.provider === 'workers' ? Boolean(env.AI) : Boolean(gem.key),
      source: how.provider === 'workers' ? 'cloudflare' : gem.source,
      bound: Boolean(env.AI),
      has_key: Boolean(gem.key),
      key_source: gem.source,
      model: how.provider === 'workers' ? PAINT_MODEL : draws.model,
      model_source: how.provider === 'workers' ? 'default' : draws.source,
      // named so the screen can show what a carousel costs before it is
      // drawn rather than after. Free on Cloudflare; there is no free
      // tier on any of the Gemini ones.
      per_image: how.provider === 'workers'
        ? 0
        : MODELS.find((m) => m.id === draws.model)?.per_image ?? null,
      choices: MODELS,
      free_daily: how.provider === 'workers' ? FREE_DAILY_NEURONS : null,
      per_image_neurons: how.provider === 'workers' ? NEURONS : null,
    },
    instagram: {
      connected: Boolean(igToken),
      user_id: (await getSetting(db, 'ig.user_id')) || env.IG_USER_ID || '',
      username: (await getSetting(db, 'ig.username')) || '',
      scopes: (await getSetting(db, 'ig.scopes')) || '',
      // the app is set up well enough to run the connect flow at all
      can_connect: Boolean(ig.id && ig.secret),
      app_id_ends: ig.id ? ig.id.slice(-4) : '',
      app_from: ig.source,
    ...
    /* Which road a post takes, and whether the other one is even usable.
       Buffer holds the connections and posts under its own apps; direct
       needs TikTok to have approved a Content Posting API application,
       which they grant on a commercial use case and not on a one-person
       business posting its own work. */
    posting: {
      route: await postingRoute(db, getSetting),
      buffer_key: Boolean(env.BUFFER_API_KEY),
    },
    /* What Spark can reach, and whether the older picture path is open.
       Neither was visible anywhere, so a deployment on the wide scope
       looked identical to one on the narrow one — and the wide scope is
       what puts a permission prompt in front of every carousel and keeps
       plan_carousel within reach. */
    agent: {
      scope: (await getSetting(db, 'agent.scope')) === 'everything'
        ? 'everything' : 'carousel',
      other_engines: String((await getSetting(db, 'agent.other_engines')) ?? '') === '1',
    },
  };
}
