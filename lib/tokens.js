...
import { apiKey as geminiKey, imageModel, MODELS } from './imagen.js';
...
export async function accountState(db, env) {
  ...
  return {
    ...
    drawing: {
      ready: Boolean(gem.key),
      source: gem.source,
      model: draws.model,
      model_source: draws.source,
      // named so the screen can show what a carousel costs before it is
      // drawn rather than after. There is no free tier on any of them.
      per_image: MODELS.find((m) => m.id === draws.model)?.per_image ?? null,
      choices: MODELS,
    },
    instagram: {
