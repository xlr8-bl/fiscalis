...
/**
 * The light/dark switch, and the script that applies the choice before
 * the page paints.
 *
 * The script has to be inline and in the head. Loaded as a file it would
 * run after first paint, and a reader who chose light would get a black
 * flash on every single navigation — which is worse than not offering
 * the choice.
 *
 * Both are exported so tools/build_theme.mjs can put the same two
 * things into index.html and book.html, which are static files rather
 * than rendered by this module.
 */
export const THEME_TOGGLE =
  '<button class="theme_toggle" data-theme-toggle type="button"'
  + ' aria-pressed="false" aria-label="Switch to light"></button>';

export const THEME_HEAD_SCRIPT =
  '<script>(function(){try{if(localStorage.getItem("theme")==="light")'
...
const SCRIPTS =
  ...
  '<script src="/assets/js/site.js?v=e6b3df08eb" defer></script>';
...
function shell({ title, description, canonical, body, ld = '', robots = '',
  ...
  return (
    ...
    robots + PRELOADS +
    '<link href="/assets/css/site.css?v=9009c31540" rel="stylesheet" type="text/css"/>' +
