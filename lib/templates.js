 ...
 */
const legalFooter = () =>
  '<footer class="jr_footer"><nav class="footer_legal" aria-label="Legal">' +
...
function shell({ title, description, canonical, body, ld = '', robots = '' }) {
  return (
    ...
    robots + PRELOADS +
    '<link href="/assets/css/site.css?v=27fc8f4602" rel="stylesheet" type="text/css"/>' +
    '<link href="/assets/css/statement.css?v=fa9ba48a14" rel="stylesheet" type="text/css"/>' +
    '<link href="/assets/css/journal.css?v=213525dc08" rel="stylesheet" type="text/css"/>' +
    '<link href="/assets/css/press.css?v=29d7fe4af0" rel="stylesheet" type="text/css"/>' +
    '<link rel="icon" href="/assets/icons/favicon.svg" type="image/svg+xml"/>' +
    `<link rel="alternate" type="application/rss+xml" title="${SITE_NAME} journal" href="/feed.xml"/>` +
    ld + '</head>' +
    `<body data-theme-section="dark" class="body jr-body"><svg width="0" height="0" aria-hidden="true" style="position:absolute" data-press-defs><filter id="pressPlate" x="0%" y="0%" width="100%" height="100%" color-interpolation-filters="sRGB"><feColorMatrix type="saturate" values="0" result="flat"/><feComponentTransfer in="flat" result="stretched"><feFuncR type="linear" slope="1.7" intercept="-0.112"/><feFuncG type="linear" slope="1.7" intercept="-0.112"/><feFuncB type="linear" slope="1.7" intercept="-0.112"/></feComponentTransfer><feComponentTransfer in="stretched" result="plates"><feFuncR type="discrete" tableValues="0.00 1.00"/><feFuncG type="discrete" tableValues="0.00 1.00"/><feFuncB type="discrete" tableValues="0.00 1.00"/></feComponentTransfer><feComponentTransfer in="plates"><feFuncR type="table" tableValues="0.031 0.929"/><feFuncG type="table" tableValues="0.031 0.125"/><feFuncB type="table" tableValues="0.027 0.141"/></feComponentTransfer></filter><filter id="pressPlatePaper" x="0%" y="0%" width="100%" height="100%" color-interpolation-filters="sRGB"><feColorMatrix type="saturate" values="0" result="flat"/><feComponentTransfer in="flat" result="stretched"><feFuncR type="linear" slope="1.7" intercept="-0.112"/><feFuncG type="linear" slope="1.7" intercept="-0.112"/><feFuncB type="linear" slope="1.7" intercept="-0.112"/></feComponentTransfer><feComponentTransfer in="stretched" result="plates"><feFuncR type="discrete" tableValues="0.00 0.50 1.00"/><feFuncG type="discrete" tableValues="0.00 0.50 1.00"/><feFuncB type="discrete" tableValues="0.00 0.50 1.00"/></feComponentTransfer><feComponentTransfer in="plates"><feFuncR type="table" tableValues="0.031 0.471 0.910"/><feFuncG type="table" tableValues="0.031 0.471 0.910"/><feFuncB type="table" tableValues="0.027 0.459 0.890"/></feComponentTransfer></filter></svg>${navbar()}${body}${legalFooter()}${SCRIPTS}</body></html>`
  );
}
...
export function renderIndexPage(articles) {
  const rows = articles
    .map(
      (a) =>
        ...
        `<span class="press-spec">${fmtDate(a.published_at)}</span>` +
        `<span class="press-spec press-spec--dim">${a.minutes} min</span>` +
        '</div></a></li>'
    )
    .join('');
  ...
  /*
   * The masthead, set as a printed one: the word oversized with the red
   * plate slipping out from under it, a slab down the margin, and the
   * count in the specimen register underneath.
   *
   * `data-over` carries the word a second time for the misregistration
   * layer. It is the same string as the heading, so a screen reader
   * would say "Notes" twice — the pseudo-element is aria-hidden by
   * being a pseudo-element, but the attribute is not read either way.
   */
  const count = articles.length;
  const body = `<main class="jr_wrap">
  <header class="jr_masthead press-grain">
    <span class="press-slab press-slab--v press-slab--spine" aria-hidden="true"></span>
    <div class="press-crop">
      <h1 class="jr_display press-huge press-over" data-over="Notes">Notes</h1>
    </div>
    <p class="press-spec press-spec--marked jr_index_spec">${
      count} ${count === 1 ? 'entry' : 'entries'} · written here, not syndicated</p>
