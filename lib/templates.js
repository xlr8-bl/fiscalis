...
export function prepare(row) {
  const parsed = renderMarkdown(row.body);
...
function navbar() {
  ...
  return (
    ...
    link('/#work', 'Work') + link('/#services', 'Services') +
    link('/#process', 'Process') + link('/journal/', 'Journal') +
    '</ul></nav>' +
...
function shell({ title, description, canonical, body, ld = '', robots = '' }) {
  return (
    ...
    ld + HEAD_WIPE + '</head>' +
    `<body data-theme-section="dark" class="body jr-body">${PANEL}${navbar()}${body}${SCRIPTS}</body></html>`
  );
}
