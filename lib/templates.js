...
function shell({ title, description, canonical, body, ld = '', robots = '',
  ...
  return (
    ...
    robots + PRELOADS +
    '<link href="/assets/css/site.css?v=61c97ec677" rel="stylesheet" type="text/css"/>' +
    ...
    ld + '</head>' +
    `<body data-theme-section="dark" class="body jr-body">${navbar()}${body}${siteFooter()}${SCRIPTS}</body></html>`
  );
}
 ...
 * @param opts.bookingOnly the site is showing one page with one thing to
 *   do, so the journal is redirecting. A sitemap that keeps offering a
 *   crawler URLs which answer 302 is a sitemap Search Console flags, and
 *   the entries come back the moment the mode goes off.
 */
export function renderSitemap(articles, { bookingOnly = false } = {}) {
  if (bookingOnly) {
    return (
