...
import { collection, sanitiseEntry, SECTIONS } from './collections.js';
...
/*
 * Hero only: the site is one screen while the work that will fill it is
 * being made. Stricter than booking-only, and it implies it — everything
 * booking-only takes out is already gone, and the navbar, the questions,
 * the closing call and the footer go too.
 */
export const isHeroOnly = (settings) => settings?.['site.heroOnly'] === 'on';

/*
 * Sections, one at a time.
 *
 * Hero only and booking only are both all-or-nothing, and the thing
 * actually wanted sits between them: keep the navigation, keep the way
 * to book, keep the footer, and take out only the parts that are still
 * showing work that does not exist yet. Neither switch can express that,
 * so this is the switch that can.
 *
 * Every one of them defaults to SHOWN. A site that has never touched
 * these settings, or has no database at all, renders exactly as the
 * markup does, which is what makes this safe to ship on.
 *
 * The list itself lives in collections.js, with the studio fields it
 * generates, because content.js already depends on that file and a
 * dependency the other way would close the loop.
 */
/** Which sections have been switched off, as the modes that remove them. */
export function hiddenSections(settings) {
  return SECTIONS
    .filter((s) => settings?.[`show.${s.key}`] === 'off')
    .map((s) => s.mode);
}
...
export async function bookingOnlyRedirect(db, { unless = false } = {}) {
  ...
  if (!settings || (!isBookingOnly(settings) && !isHeroOnly(settings))) return null;
