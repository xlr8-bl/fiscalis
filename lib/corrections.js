...
/*
 * The home page title, which shipped with an em dash in it.
 *
 * This lived in SCHEMA for one commit as an UPDATE with a LIKE in it, and
 * D1 answered "LIKE or GLOB pattern too complex" and failed the whole of
 * Set up — a data fix taking the table creation down with it. It belongs
 * here: a correction is parameterised, matches byte for byte so it cannot
 * walk over an edit, and a failure is swallowed rather than fatal.
 */
const TITLE_NOW =
  'Web3Ashley: web design, development, and the analytics to prove it worked';
const TITLE_WAS =
  'Web3Ashley \u2014 web design, development, and the analytics to prove it worked';

export const CORRECTIONS = [
  { key: 'seo.title', was: TITLE_WAS, now: TITLE_NOW },
  {
    key: 'gap.heading',
...
/**
 * Small, exact fixes inside an article that is already published.
 *
 * The two lists above replace a whole value, which only works while
 * nothing has been edited — right for a setting, useless for an article,
 * because a body that has had one word changed since would never match
 * again and would keep the mistake for good.
 *
 * So these are substring swaps. `was` has to be specific enough that it
 * could not appear anywhere a person meant it to: a fragment like "1–3%"
 * is not something anyone types on purpose in a body that also carries
 * the corrected form. That is what keeps the promise the other two make
 * — never walk over an edit — without needing the body to be untouched.
 *
 * These five are en dashes in numeric ranges, from applying the
 * humanizer skill (.claude/skills/humanizer) to the journal. Its dash
 * rule is absolute, and "1% to 3%" reads the same as "1–3%" in a
 * sentence somebody is meant to read rather than scan.
 */
export const ARTICLE_TOUCHES = [
  { slug: 'analytics-that-actually-matter',
    was: 'A useful reference: 1–3% is normal',
    now: 'A useful reference: 1% to 3% is normal' },
  { slug: 'rebuild-or-repair', was: '**5–6 yes.**', now: '**5 or 6 yes.**' },
  { slug: 'rebuild-or-repair', was: '**3–4 yes.**', now: '**3 or 4 yes.**' },
  { slug: 'rebuild-or-repair', was: '**0–2 yes.**', now: '**2 or fewer yes.**' },
  { slug: 'what-delivery-apps-cost-you',
    was: 'roughly 1.5–2.5%', now: 'roughly 1.5% to 2.5%' },
];

/**
 * Apply those, where the old fragment is still in the body.
 *
 * @returns {Promise<string[]>} the slugs actually touched
 */
export async function applyArticleTouches(db) {
  const touched = [];
  for (const { slug, was, now } of ARTICLE_TOUCHES) {
    try {
      const result = await db
        .prepare(
          `UPDATE articles
              SET body = replace(body, ?1, ?2), updated_at = datetime('now')
            WHERE slug = ?3 AND instr(body, ?1) > 0`
        )
        .bind(was, now, slug)
        .run();
      const changed = result?.meta?.changes ?? result?.changes ?? 0;
      if (changed > 0 && !touched.includes(slug)) touched.push(slug);
    } catch {
      // no articles table yet
    }
  }
  return touched;
}
...
/**
 * Facebook was a posting target and is not one any more: its poster was
 * never implemented, and there is no Facebook channel connected.
 *
 * A carousel filed while it was still on the list still names it in
 * `targets`, and the rehearsal correctly refuses a target nothing can
 * reach. Stripping it is a repair rather than a decision — the carousel
 * still goes exactly where it can go.
 */
export async function dropRetiredTargets(db) {
  try {
    const out = await db
      .prepare(
        `UPDATE carousels
         SET targets = TRIM(REPLACE(',' || targets || ',', ',facebook,', ','), ','),
             updated_at = datetime('now')
         WHERE ',' || targets || ',' LIKE '%,facebook,%'`
      )
      .run();
    return out?.meta?.changes ?? out?.changes ?? 0;
  } catch {
    return 0;   // a database without the table yet has nothing to repair
  }
}
