...
export function offeredPlatforms(setting) {
  ...
  /* WhatsApp sits in the default set because in most of the world it is
     the way somebody expects to be called, and because one number takes
     both it costs nothing to offer alongside the phone. */
  const list = want.length ? [...new Set(want)] : ['meet', 'zoom', 'whatsapp', 'phone'];
