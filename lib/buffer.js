 ...
 *
 * Buffer's Instagram metadata has TWO non-null fields, and sending one
 * of them refuses the whole document rather than that one post: a
 * variable that fails coercion fails the request, so both platforms came
 * back with an Instagram error.
 *
 *   type                PostType. post, story or reel. What the post IS.
 *   shouldShareToFeed   whether it also goes to the feed.
 *
 * `shouldShareToFeed` is false on a story on purpose. True would put
 * slide one in the feed as well as in the story, which is a second
 * public post of the cover that nobody asked for.
 *
 * TikTok's metadata is two fields, `isAiGenerated` and `title`, and
 * both are optional, so nothing is sent for it. Neither is privacy: a
 * SELF_ONLY post cannot be asked for through Buffer at all, which is why
 * the rehearsal on this road is a draft rather than a private post.
 *
...
const metaFor = (target, { story } = {}) => (target === 'instagram'
  ? { metadata: { instagram: {
      ...
      shouldShareToFeed: !story,
    } } }
  : {});

export async function toBuffer(env, {
  urls, caption, targets, scheduleFor = null, story = false, draft = false,
  ...
  send.forEach((s, i) => {
    ...
    const now = !draft && !future && i * gap === 0;
    s.at = draft || now ? null : new Date(at).toISOString();
    /* A draft is Buffer's own rehearsal: status `draft` rather than
       `scheduled`, and nothing publishes until a person opens Buffer and
       schedules it. `addToQueue` is the mode the draft example uses and
       dueAt is not taken alongside saveToDraft. */
    /* `needsApproval` is non-null on CreatePostInput and there is one
       person here, so it is false. Omitting a non-null field is how the
       first real call failed. */
    const common = {
    ...
    s.input = draft
      ? { ...common, mode: 'addToQueue', saveToDraft: true }
      : { ...common,
  ...
  send.forEach(({ target, key, channel, at }, i) => {
    ...
    out[where] = {
      ...
      ...(at ? { at } : {}),
      // nothing is public yet, and the result has to say so or a draft
      // reads as a post that went out
      ...(draft ? { draft: true } : {}),
    };
  });
...
/**
 * Which road a post takes.
 *
 * A setting rather than "is the key present": switching back has to be a
 * decision, not a side effect of deleting a secret.
 *
 * Buffer is the default because the direct road's TikTok half is not
 * available to this deployment. TikTok refused the Content Posting API
 * application: their own terms require a commercial use case, and a
 * one-person business posting its own work is not one they grant. So an
 * unset route pointing at `direct` pointed at a road half of which
 * cannot work, and the symptom was a message about making the account
 * private, which is an unaudited-client rule that no longer applies to
 * anything here.
 *
 * `direct` is still reachable by setting it, and Instagram still works
 * on it.
 */
export async function postingRoute(db, getSetting) {
  const v = String((await getSetting(db, 'post.route')) ?? 'buffer').toLowerCase();
  return v === 'direct' ? 'direct' : 'buffer';
}
