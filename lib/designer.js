...
export function designBrief() {
  return {
    ...
    /*
     * The teaching carousel is a different job with a different engine,
     * and it is listed HERE rather than behind its own brief because the
     * choice between them is the first one to make: a hook sheet stops a
     * scroll, a teaching slide explains something to somebody who has
     * already stopped.
     */
    teaching: {
      use_when: 'The post explains something: a check somebody can run, why a '
              + 'thing goes wrong, what two options actually differ by. Pick a '
              + 'template per slide and write the copy for its blocks. Never '
              + 'give a coordinate — the geometry is measured and yours to fill, '
              + 'not to invent.',
      tool: 'teach_carousel',
      ...slideCatalogue(),
      /* How to use it, not just what exists. A schema says what is
         allowed; this says what is good, and the difference between a
         slide that validates and one worth posting is all in here. */
      guide: slideGuide(),
    },
    rules: [
 ...
 */
/* The outros. `close` is a teaching panel with a headline on it, and it
   is what gets reached for when nothing insists otherwise — which is why
   three carousels in a row ended on one. */
...
export async function fileTeaching(env, spec, { carouselId, slug }) {
  ...
  if (given.length && !given[0].hook) {
    shape.push(
      ...
      + 'names which sheet to use and design_brief lists all twelve under '
      + 'hooks.sheets, each with the slots it takes.'
    );
  }
  const last = given[given.length - 1];
  if (given.length > 2 && last && !OUTROS.includes(last.template)) {
    shape.push(
      `The last slide is "${last.template ?? 'nothing'}" and an outro is one of `
      + `${OUTROS.join(', ')}. close is a middle panel with a headline on it; it `
      + 'does not end the set. bookend answers the opening, recap lists what was '
      + 'checked, calling names who it was for. next_carousel says which is next.'
    );
  }
