 ...
 */
/**
 * Every rule, run before anything is written.
 *
 * It exists because the route INSERTED the carousel row and then called
 * fileTeaching, which could still refuse — on the hook-sheet rule, or on
 * a repeat. The refusal was returned and the row stayed, so every failed
 * attempt left a carousel with no slides under it. With the schema
 * missing a `hook` field there were a great many failed attempts.
 *
 * Same checks fileTeaching runs, so nothing can pass here and fail there.
 */
export async function checkTeaching(env, spec) {
  ...
  if (!given[0]?.hook) {
    shape.push(
      'Slide 1 has to be a hook SHEET, not a teaching template. Send `hook` with '
      + 'a sheet id and the sheet\'s slot names as fields beside it — not '
      + '`template`. next_carousel returns the exact shape to send in '
      + '`slide_1.send_exactly_this_shape`, with a character ceiling per slot.'
    );
  }
  ...
  return shape.length ? { ok: false, errors: shape } : { ok: true };
}
...
export async function fileTeaching(env, spec, { carouselId, slug }) {
   ...
   * ending.
   */
  const again = await checkTeaching(env, spec);
